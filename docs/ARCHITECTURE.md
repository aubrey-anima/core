...
>
> 🔴 **这一行原先写着「未来规划在 ROADMAP.md。对应引擎版本 1.0.0(db 格式 1)」,两半都是假的**
> (2026-08-25 改)。[ROADMAP.md](ROADMAP.md) **不是未来规划,是 1.0.0 之前的设计存档**,
> 它整份建立在「主版本号 = db 格式」上,而**「db 格式」这个东西 2.0 那一轮就删掉了**
> (world.db 整体退役);照那个指路牌去读的人,会拿一份死掉的坐标系判"什么做了没做"。
> 「对应引擎版本 1.0.0」同理:今天敲 `python -c "import anima_world; print(anima_world.__version__)"`
> 答的是 **3.7.0**。⚠️ **而我没有替这份文档盖一个新的版本号** —— 逐节核过它到底对到哪一版
> 才盖得上,没核就盖等于把一句烂了的话换成一句新的烂话。**当前实现以
> [REFERENCE.md](REFERENCE.md)(公开方法与配置键有测试闸盯着)与 [CHANGELOG.md](../CHANGELOG.md) 为准。**
>
...
- **聊天子系统与事件核解耦。** 整场会话只在关闭时发一个事件;完整转录留在宿主应用里,
  不落世界。世界只收当轮有限历史。
- **坏节拍脚本必须在加载时当场报错**,不能流到世界启动。显式 `--seed` 同理
  (`WorldSeedError`);只有内置种子降级到硬编码默认。
