...
没有位置有**三种**可能(和 `presence` 那条命令、和答话那扇门的闸表里那格
`player_where_unknown` 逐字同一份 —— 那一格在
`git grep -n player_where_unknown -- docs/`,别照章节号找,号会烂):① 他 `player_leave` 过,自己走了;② 他很久没动静,在场记录过了 15 分钟
没续上;③ 宿主确实没落过 `player_move`。**前两种里宿主刚刚才调过** —— 只说第三种,
运维就会照着一个不存在的故障去查一个接得好好的宿主。
