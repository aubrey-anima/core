...
kind → 分型的默认映射在 `Scheduler.PROVENANCE_BY_KIND`,**没列的一律亲历** ——
...
**幂等**(第二次跑各格都是 0);不存在的 `--world-id` 一律**退出码 2**。
...
| `--baseline` | 6 | 基线取她最早的几条 |
| `--json` | - | 见 `World.persona_drift()` 的返回值 |
...
| `chat.persona_anchor.enabled` | bool | false | **人设尾部重注**(R1,§2.9.12):在整段提示词最末再压一块「你是谁」。人设块坐在开头而注意力随窗口填满而衰减,八轮内就测得出漂移 —— 这一手不换模型、不加调用,只是把她是谁挪一份到她最后读到的地方。**默认关**(它改变她说话的形状);内置种子点亮了它。模板 `chat.persona_anchor` |
...
| `memory.consolidation.enabled` | bool | false | **夜间固化**(R5,§2.9.13):日切时衰减一轮、清扫弱到召不回的行、让攒够的去反思。挂在日切而不是每 tick ——遗忘曲线按世界日算,而反思是一次 LLM 调用,跟白天的对话一起跑会抢线程。**这个世界自己有夜晚**,不必假装 |
