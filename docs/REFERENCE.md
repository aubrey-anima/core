...
```
  ! 要花时间的长过程:起了 12 件,做完 11 件,1 件半路被带走(代价不退)
      最近一件:夏 的 夜播 bench:oak
      做完 11 / 被带走 1 —— 比例低就是排班在抢她的手,看 occupies 声明对不对
```
...
没有位置只剩两种可能:宿主没调过 `player_move`,或者他很久没动静已经过期
(`world.player_ttl_seconds`,默认 15 分钟)。
...
⚠️ **但第二种那句回执自己也还欠一笔**(2026-08-20 记):它把原因写死成
「宿主没调过 `player_move`」,而**下线**(`World.player_leave`)和**在场行过期**
(15 分钟 TTL)落的是同一支 —— 那两种情形里宿主刚刚才调过。邀请门那半边
(`answer_invitation` / `agent_invites`)这一轮已经改成按闸分家、不再指认宿主;
**这扇门(`presence.enforce_colocation`,默认关)还没改**,改它要动一条钉着这句话
的测试断言,留给下一轮。读这一格时按「世界不知道你在哪」理解,别按「宿主没接」理解。
