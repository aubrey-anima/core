...
- **人和 NPC 共用同一套行动。** 玩家此前只有两个入口,而 `World.player_action()`
  ...
  写在引擎里、它那侧看不见。
  🆕 **3.10.0(批 1.1 ④):做成了的 `interact` 一定带一句人话。** 回执里多一格
  `said`(「你报到了狮心会。」),`ToolResult.text` 就是它。**即使 `changed` 是空的**
  —— 真站实测:玩家点「报到狮心会」,那条能力一个量都不改,于是回执里只有一堆空
  dict、`text` 空着,屏幕**纹丝不动**,和什么都没按一样。同一句话也进叙事日志
  (`narrative` 事件,`payload.source == "template"`):它是**地板**,一次 LLM 都不调;
  作者写了 `importance` 且 `narrative.player.enabled` 打开时,那条 LLM 旁白照旧,
  是**升级**。⚠️ `said` **只给玩家** —— 角色那条路的回执进的是她的对话流,
  多一句「你…了…」会变成她自己念出来的一句旁白。
...
| `world.host_turn(player_id, *, ask=False)` | **主持人的那一屏**(3.9.0,§9.2):一段场景 + 至多 `host.max_options` 个选项(有多少递多少)+ **永远在最后**的自由输入。`{"player_id","tick","day","place","place_name","trigger","scene":{"text","source","seq"},"options":[…],"ask_ready_tick","ask_ready","ask_ready_text","blocked","blocked_text"}`;一项 = `{"id","kind","label","who","hook","tone","available","reason","refusal","cost","door":{"method","params"}}`。`who` 是**这一项指着的那个人**(人名,给宿主拼文案用):`talk` / `invitation` / `beat` 三类非空,`verb` / `travel` / `free` 是空串 —— 它和 `label` 是两样东西,`label` 是一句祈使(「和苏晚夏说说话」),拿它当人名拼会出「这儿有人:和苏晚夏说说话」。⚠️ **这两张键表都是机器校验的**(`tests/test_host_doc_contract.py` 对着 `contract --json` 的 `host.turn_keys` / `host.option_keys` 逐格比),所以下一格插进来时这一行会当场红 —— 上一次它没红,下游三处(站点 README / `PlayerHostOption` / 后端夹具)就照着这一行同缺了一格。**一个方法不是四个** —— 说场景与递选项必须同一次取,分开就是对一个动着的世界取两次快照(场景说「他在门口」,选项里他已经走了,而这种不一致一处都不报错)。🔴 **主持人是荐者不是执行者**:每一项的 `door.method` 是闭集(`answer_invitation`/`chat`/`player_walk`/`player_tool`/`free`),点下去走的全是今天已经存在的那扇门 —— 这一层一条新的「写世界」的路都不开。每种门的 params 键集在 `contract --json` 的 `host.door_params` 里,**消费方按段对表,别按名字猜**。`available`/`reason`/`refusal`/`cost` 是 `player_options` 那四格**原样透传**。**只在六个时刻开口**(`arrive`/`new_day`/`beat`/`acted`/`ask`/`return`),闸在这个函数里:时刻钥匙 `(place, day, 指着他的最后一条 beat_fired 的 seq)` 对上就原样返回上一屏(`scene.source == "cached"`);`ask=True` 另受 `host.ask_cooldown_ticks` 管,而那个冷却**也带在返回里**(`ask_ready_tick`/`ask_ready`)—— 宿主够不着 `contract` 时照样置得了灰。🔴 **`card.billing == "hidden"` 的角色不进候选,也不进给模型的那份提示**(不是"给了再叮嘱它别说":这一屏交出去的是散文,宿主筛不了,筛一半比不筛更坏)。场景走**背景槽**,一次调用、失败即模板(`host.mock_scene`)、**不重试不合批**;**挑哪几项是纯算术**,同一个世界同一时刻挑两次逐项相同。开口那一下落成一条 `host_scene` 事件,那一屏是它的投影(所以刷新之后开场还在)。CLI 出口 `anima-world player host` |
| `world.player_options(player_id)` | **这个人此时此地点得动什么** —— 可以直接渲染成按钮的那一份(§2.9.6)。`{"player_id","location","location_name","blocked","blocked_text","overflow","own":{"quantities","readouts"},"targets":[{"id","name","gloss","kind","quantities","readouts","verbs":[{"verb","label","available","reason","refusal","cost","participants","candidates"}]}]}`。`player_perception` 答"这儿有什么、能被怎么做",这一条再往前一步答"**这会儿点得动吗、点不动是为什么**"。`readouts` 每行 `{"key","label","value","word","unit","text"}` 是**量该怎么印**:`quantities` 那份键与数字仍是契约(拿去判断、画曲线),`readouts` 是加上去的人话 —— 措辞走 `perception.readout_text`,**和她提示词里那一行是同一个函数**(各写一遍的下场是她读到「雨势 瓢泼大雨」而玩家读到 `雨势 0.82`)。`label` 永远印得出来(作者没写就是量名本身),`word` 是分档的量落在哪一档(没分档是空串)。`own` 是**他自己身上的量** —— 没有它,`incapable` 那句"你还差点什么"是句哑谜。`cost` 是**按下去之前**那句话(「要花 1 小时,这期间做不了别的」;一下子完的事是空串)—— 四类拒绝管的是点不动的按钮,而**一个把人锁住一小时的按钮是点得动的**,不说的话玩家点完才知道。**只说时间**:量和材料不够时 `incapable` 会当场点名,而时间总是够,于是永远没人拦他。⚠️ **不新造第二套真相**:成不成走 `perform_affordance(dry_run=True)`,和真点下去那一次是同一个函数;四类拒绝(`conditions`/`incapable`/`busy`/讲不通的那摞)一个都不合并。⚠️ **一个字节都不写** —— 它每一帧都要被渲染一次。`blocked` 是空菜单的**原因**(`unknown_player_location` / `in_transit` / `no_ontology`),不是一句沉默;`blocked_text` 是同一个原因的**人话**(3.3.0 起就在返回值里),照着印就行 —— 🆕 **3.9.0:`blocked` 只挡 `targets`,不挡 `own`。** `own` 那两格不依赖本体层、也不依赖他站在哪 —— 量住 `stock:agent:player:<id>`,档词与人话名字走**可见性表**。从前它挂在 `no_ontology` 早退后面,于是一个用插件给玩家挂了 `role.评级`(Redis 里真有、可见性表里真有 bands)而没写 `kinds` 的世界,`own.readouts` 是 `[]`、宿主的状态四格一片空白;随手加一个 `kind`,同一份插件立刻出「评级 预科」。⚠️ 连带一句老话作废:**"被挡住时 `own` 本来就是空的"从 3.9.0 起不成立了** —— 三条 `blocked` 路径上 `own` 照样有值(那次"玩家看不到自己身上任何一个量"的误诊,真相是 `blocked=unknown_player_location`,而现在连那一格都不会空了) |
...
| 🆕 `world.install_pack(path, *, force=False)` | 把一份**内容包**投进这个正在跑的世界(3.10.0)——「每周有更新」那条路。`path` 是一份带 `pack` 段的 `.cyberworld`。**一个文件就是一个 pack**。这条路新开的三段:剧情拍(按 id 合并,**零点是这个包落地那天**)/ 作者动过的开关 / 世界观;其余段照旧只填缺不覆盖,走开机那条路上同一批播种函数,而**缺席的段 = 不动**(不是回落引擎内置那份 —— 那个回落只对创世成立)。回执**十七格,那张表只有一处** —— §4.11「回执键表」(`tests/test_pack_doc_contract.py` 拿真回执逐格比着它;此前它有三个数,而下游抄的就是这几处)。`skipped` 说的是**「这一趟没写下去的那几样」**:`personality` 记「**不必写**」那一种(这份包想写的那句,世界里已经是了),`memories` 记「**她已经记着了**」那几条 —— ⚠️ **不是「被拒了的」**(被拒时不带 `--force` 整份 rc 2、带了就照写,两种都不会留在这张表上)。装不进抛 `PackInstallError`,而且**一个字节都没写**(判断全在写之前,插件降级那一道也在)。`force=True` 放行的是**两件**事,而不是一件:① 写了 `since: "world"` 而 `day` 已经过期、装上去会在下一 tick 一起烧掉的那几拍;② 在册的人的人设 CAS 冲突。回执上 `forced_beats` / `forced_personality` **分开说强的是哪一件** —— 只有一格时,屏上那句解释必然对其中一种情形说假话。🆕 **2a-②**:在册的人的 `personality` 按 **compare-and-set** 覆盖 —— 判据不是「我是同一个 pack」,是「这一格此刻的值还等于我上一版写下去的那个值吗」;不是我写的、或者写下去之后被世界改过 → **默认拒绝**(`force=True` 才写,回执 `forced`)。在册的人的 `memory` **只增不改**(按 `(agent_id, summary)` 去重)。⚠️ 停用与启用是另外两扇门(`disable_pack` / `enable_pack`,3.10.0 / 3.10.1)。回执那张表见 §4.11「回执键表」——**十七格,只有那一张**。CLI 出口是 `anima-world pack install`(§4.11) |
...
| 🆕 `player_options().person_verbs` | **玩家菜单上那一族对人动词**(3.12.0,player 带回):每个**在场的人**一组 `{agent_id, agent_name, verbs[]}`,每个 `verbs[]` 的键见 `contract.person_verbs.options_keys`(`verb` / `label` / `consent_mode` / `available` / `reason` / `refusal`)。🔴 **`available` 的意思是「问得出口」,不是「她会答应」** —— 它过的只是世界那几条硬闸(不在这儿 / 睡着了 / 手上有事 / 把你静音了);**她肯不肯是真门那一下才知道的**(`willingness` / `judge_invite`),这一层先替她答一遍就是**把她的否决权挪到菜单上**。⚠️ **不依赖本体层**:一个没写 `kinds` 的世界(`blocked: "no_ontology"`)照样递得出它 —— `blocked` 该挡的只有 `targets`(和 `own` 那一格逐字同一课)。补这一格之前,声明有、门有,而**玩家点不到**:`targets[].verbs[]` 是 affordance 那一族、工具目录那两个参数不枚举、主持人那屏也不递 |
| 🆕 交织(批 3c,3.13.0) | **玩家 A 的选择成为玩家 B 的事件** —— 而**两条线仍然各是各的**。总口径:交织的价值在「你的选择被另一个人撞见」,**不在「两个人共享一份剧情」**(后者要一份跨玩家的故事状态 = 第二份真相;前者**一格新的故事状态都不要**)。哪几种事会被撞见:`contract.director.crossing_event_types` —— 🔴 **第三张策展表,有意不复用那两张**(`PLAYER_MOVE_EVENT_TYPES` 答「他自己动没动手」、`RECAP_EVENT_TYPES` 答「该跟他说哪几件事」、这一张答「别人做的事里哪几种他该看见」;为「两处各写各的」栽过三次,所以它显式写下来并配逐格用例)。⚠️ 收的是**看得见的动作**:`payment`/`item_transfer`/`director_log` 一条都不在里面 —— 站在旁边的人看不见别人的账,放进来就是**拿交织当剧透**。⚠️ **同地才算**(同一个 point,不放宽到父节点)。时刻钥匙第四格 `here_seq` 读投影的 `place_move_seq`(按**地点**记、不按人记,且**只收玩家做的** —— NPC 每 tick 都在动,算进来会让屏幕永不停歇地重开) |
| 🆕 玩家面事件的投递(批 3c §2.2) | 🔴 **别人的 GM 笔记不出门。** `director_log` 与三条 `*_settled` 是**写给某一个玩家的**,进插件触发器时:① **剥掉 `why`**(⚠️ 剥的是**交给触发器的那一份**,`history` 里那条一个字没少 —— 就地改载荷会让日志和它自己的投影对不上);② **效果只许作用于当事人** —— 跨玩家的 `link`/`unlink`/`transfer` **当场拒**并落 `director_log_refused`(`reason: "cross_player"`)。**拒了必须留痕**:静默拒绝会让作者以为那条规律在跑,而它每次都被吞掉。⚠️ 这个口子是**实测出来的**(把 `to` 写死成另一个 `player:<id>`,边真的连上了);判据是**两端解析之后的节点**,而且 `player:` 与 `agent:player:` 两种前缀都认 —— 只认一种的闸换个写法就绕过去了。契约:`crossing_event_types` / `private_payload_keys` / `player_facing_events` / `cross_player_refused_event` |
| 🆕 `world.player_clues(player_id)` | 他的**线索板**(3.13.0,批 3c §2.4):`{total, known, unknown, known_ids, text}`(键表以 `contract.clues.options_keys` 为准)。🔴 **只报存在与解锁状态,不报内容**(设计 §5 那张表的原话):已知的那几条给 id(他本来就知道),**未知的那几条连 id 都不给** —— 一个未解锁线索的 id 往往**就是它的谜面**。「他知不知道」**没有第二处状态**:就是 `clues.knows` 那条边在不在。🔴 解锁**只有三条路**(`contract.clues.unlock_paths`:动词 `effects` 的 `link` / 剧情拍的 op / 编剧的 `reveal`)—— **没有第四条**,别等一个不会来的作者判定。⚠️ **不报「怀疑」那一档**:它要置信度,而置信度是判定那一族的东西(批 4)——**报一个算不出来的档,就是让屏幕替引擎撒谎**。⚠️ 不依赖本体层能不能用(和 `own` / `person_verbs` 同一课)|
| 🆕 `world.player_faction(player_id)` | 他站在哪一边、声望到哪一档(3.13.0,批 3c §2.6):`{faction, faction_name, standing, standing_text, text}`(键表以 `contract.factions.options_keys` 为准)。「他站在哪一边」**没有第二处状态**:就是 `factions.member_of` 那条 `exclusive` 边。🔴 **「站了就回不去」是内核在 `link` 那一刻查的**,不是这一格判的 —— `exclusive` 放行的样子是安静的(两条边同时挂着,`plugin list` 看不出来,而提示词里他同时属于两个阵营)。🔴 **没有「这一边有几个人」那一格**:计票 / 人数上限 / 禁地 / 全城声望四样全是聚合或 gates,是批 4 的东西,`contract.factions.deferred` 把它们点名 —— **在一个没有聚合的引擎上手写一遍计票,就是把批 4 那件事做进这一层**,而那份实现以后要被删掉重写,并且在被删掉之前它是**第二份真相**。`standing_text` 的分档词**只有一份表**(和 `tension_text` 同一条:各译一遍的话,同一个数会在引擎屏 / 创作台 / 站点上说三种话)|
...
| 🆕 `world.guidance()` | 给实时编剧的那份**指导**(3.11.0,作者层的一个段 —— ⚠️ **不写第几个**:段分三张表,序数从落笔那天起就会烂,要数敲 `contract --json`;CLI 出口 `anima-world guidance show [--json]`,**只读** —— 换一份走 `pack install`):`{themes, cast_pool, forbidden{agents,locations,ops,text}, tone, pacing{ceiling,target_curve}, arcs[{id,milestone,steer}]}`。老板 2026-09-02:「这个周更只能算是指导」—— 它不是剧本,**主线由玩家自己走**。`{}` = 这个世界没写过指导,而**那不等于「编剧缺席」**:开关是 `director.enabled`,没写指导时编剧按保守默认跑(只 `breathe` / `approach`,人物池 = 全世界减藏起来的与被硬闸拦下的)。⚠️ 这一条和 perception / 本体层那句「声明本身就是开关」**有意不同**。🔴 `forbidden` 前三格是**闸**(引擎按它筛人、拦 op),`text` 只进提示词 —— **一句写在提示词里的禁令不是闸**。`arcs[].milestone` 用**拍那一套谓词**,不新发明一门表达式。🆕 **3.11.1**:`pacing.target_curve` 认的是 `contract.director.target_curve_keys` 那**三**个相(`setup` / `escalation` / `climax`)—— 🔴 **`release` 不许写进去**:它是一条线**走完之后**的样子,**没有时长**;拿 `phases` 那四格去判会**多放一格**,而作者写下去**不报错也不生效**(引擎从 3.11.1 起当场拒)。换一份走 `pack install`(整份覆盖);`--world-file` 那条路只填缺不覆盖 |
...
顶层**二十**段:`beats` · `character_card` · `chat_tools` · **`clues`** ·
**`config`** · **`director`** · `engine_version` · `erasure` · **`factions`** ·
**`host`** · `invitations` · `operation` · `package` · **`packs`** ·
**`person_verbs`** · **`plugins`** · `report` · **`roll`** · `seed` · `storage`。
...
`world_setting` `guidance` `personality` `memories` `agents` `locations` `skipped`
`forced` `forced_beats` `forced_personality`
—— **十七格,而这一行是唯一的那张表**(`tests/test_pack_doc_contract.py` 拿**真回执**
...
| `host.max_options` | int | 5 | 主持人一屏**至多**递几个选项(世界里有多少就递多少,少的时候就少)。**自由输入那一项不占名额**(把它算进去,选项一多"我想说点别的"就被挤掉,而那正是这一层要保住的自由) |
| `host.ask_cooldown_ticks` | int | 12 | 他点「我该干嘛」之后,隔多少 tick 才会**重新生成**一段场景。没到就原样返回上一屏(`scene.source == "cached"`)—— 不然连点十下就是十次 LLM 调用。这个数**也带在 `host_turn` 的返回里**(`ask_ready_tick`),因为宿主未必够得着 `contract --json`。🆕 **3.10.0(批 1.1 ③):`arrive` 那一屏之后的第一次问不起冷却** —— 龙族这一格是 `12`,而一 tick 是 5 真实分钟,于是一个刚进门的新玩家那颗按钮**整整 60 真实分钟按不动**,而那正是他最需要按它的一个小时;冷却防的是「连点十下 = 十次 LLM」,进门第一次问不是那件事(契约里那一格是 `host.ask_free_after`)。同轮返回多一格 **`ask_ready_text`**(「还要约 N 分钟」)—— 把 tick 折成真实时间要读 `scheduler.tick_rate`,而宿主够不着配置,**让每个宿主各算一遍就是让它们各持一份对时钟的猜测,而猜错了不报错** |
| `host.away_ticks` | int | 288 | 🆕 **3.10.0(2a-②)**:他离开多久之后,下一屏是「你回来了」(`trigger == "return"`)—— 默认一个世界日;`0` = 永远不当他离开过。⚠️ **两个条件都要**:他刚才**真的不在**(在场行带 TTL,过期了才算),而且世界走了这么久。少了前一条,一个一直在玩的人会被告知「你有 1 天没来了」——**「世界走了多久」不是「他离开了多久」**,而这两个数在屏幕上长得一模一样。另一条触发不看时间:他上一屏之后有新的 `pack_installed`(按 `seq` 比,不按 `tick` 比 ——装包和上一屏可以在同一个 tick 里) |
| `director.enabled` | bool | **true** | 🆕 **3.11.0(批 3a)**:**实时编剧**。开着时,玩家**每做一件事**,主持人那一屏的上半场就多一拍剧情(闭集 `breathe`/`approach`/`invite`/`reveal`/`complicate`)。🔴 **不许沉默**:超上限 / 同意门拒了 / 锚点刚响过都退成一句 `breathe` 模板句,没配 key 时整条 mock 路照跑。🔴 **3.11.2 起默认开** —— 这是「引擎默认值全关」那条规矩**有意的例外**:编剧不是一个可选特性,它就是这一版的产品命题,而且它没配 key 也照跑,所以默认开不会让任何世界因为缺 key 而变坏。实测过默认关的下场:三个世界升上去之后**一拍都没写**,而每一块屏幕都显示正常。不要它的世界显式写一行 `director.enabled=false` |
...
| `director.first_arc_beats` | int | 6 | 🆕 **3.13.0**(真站第七轮 ⑤):一个玩家的**头 N 拍**里,相位的目标张力按比例压低,好让这条弧在**第一小时之内**走到第一个节骨眼(于是至少有一次 `confront` 或 `reward`)。🔴 真站实测:新玩家那 13 条 `director_log` 里 `confront`/`reward`/`callback` 与 `roll` **全部零触发**,张力 0.05 —— **3b 那整套玩家面对一个新玩家等于不存在**。⚠️ **压低只影响「翻不翻篇」,绝不影响「写多大」**:`gap = 目标 − 张力`,目标压低 = gap 变小 = 动作变小,而 `confront` 恰恰要 `gap >= 0.30` —— 第一版两处都压,那一改**把摊牌关掉了**(`pick_move` 读满目标,`next_phase` 读压低的,闸在 `test_压低目标只影响翻篇_不影响写多大`)|
| `clues.enabled` | bool | false | 🆕 **3.13.0**(批 3c §2.4):线索那条边(`clues.knows`,玩家 → `entity:clue`)。**零新原语** —— 一条线索是一个实体,「他知不知道」就是这条边在不在,没有第二处状态。🔴 线索板(`world.player_clues`)**只报存在与解锁状态,不报内容**。**默认关**:一个没写线索的世界不该多一条它从不使用的边类型 |
| `factions.enabled` | bool | false | 🆕 **3.13.0**(批 3c §2.6):阵营那条 `exclusive` 成员边(`factions.member_of`,`agent:player:<id>` → `group:<实例名>`;**这条边上只放开 `link`**,`unlink`/`transfer` 关着 —— 「站了就回不去」是产品裁决,放人走在批 4)与两格量(每人 `factions.声望`、世界级 `factions.士气`)。哪几个阵营是**作者写的**;出厂只声明边与量,不声明具体阵营。🔴 **不含计票 / 人数上限 / 禁地 / 全城声望** —— 四样全是聚合或 gates(批 4),`contract.factions.deferred` 点名。**默认关**:一个没写阵营的世界不该多一条它从不使用的边类型 |
| 🆕 `director.DIRECTOR_MAY_DEDUCT` | **编剧自己动手时只扣得动这几种赌注**(3.12.1,验收 A ①):今天是 `relation` 一种。🔴 3c 那条代拍写着「只掉关系与声望」,而**它从来没被写成代码** —— 真站上一次 `confront` 输掉,玩家的钱 **160 → 130**。**一条写下来的裁决和一行没写的代码之间,只有屏幕上那笔钱知道差别。** 分界是「谁按下的」:`money`/`item` 是玩家攒出来的东西,拿走它得由一个**他按过的动作**负责(动词的 `costs`/`consumes`、一次交易);`relation`/声望是**世界对他的看法**,那才是编剧该动的。⚠️ **两个方向都挡** —— `reward` 那一侧(还回去)也一样:**引擎凭空给钱和凭空扣钱是同一种越权**,而「还回去」更难被发现。⚠️ 模型照旧**押**得了 `money`(那句话在故事里是真的),只是引擎不替它扣;要真扣就写一个动词让玩家自己按下去 |
...
| `unlink` | `type` | 🆕 **3.13.0:断一条边。** 给了两端就断那一条;只给一端就把这一端上这个类型的边**全断掉** —— 所以必填只有 `type`。🔴 **公共边上不是每条都断得动**:照 `contract.plugins.shared_edge_ops` 判 —— `factions.member_of` 上 `unlink`/`transfer` **是关着的**(「站了就回不去」是产品裁决 3c §2.10 ④),加载期当场拒、运行期也不进 `ops_applied`。⚠️ 这一行上一版拿「退出师门」当**推荐写法**举例,而那正是被关掉的那件事 —— **一份把关掉的路当功能推荐的文档,比没有那一行更坏**。放人走在批 4(`contract.factions.deferred` 的 `leave`)|
...
`once`;3.10.0 加 **`narrate`**)。3.8.0 一个都不查 —— 所以**写了 `for_each` 的包在
...
```jsonc
{"id": "报到", "for_each": {"node": "player"},
 "trigger": {"at": {"day": 0}},
 "narrate": "一封烫金的录取通知躺在你桌上,信封上印着卡塞尔学院的狮头。",
 "payload": [{"op": "grant_item", "agent_id": "player", "item_id": "通知书"},
             {"op": "pay", "from": "__world__", "to": "player", "amount": 800}]}
```

**为什么引擎不自己编这句话**:主持人那一屏想说「刚发生了什么」,可它手上**只有 op 名**
(`memory` / `pay` / `grant_item`)—— 把「这一拍响了」翻译成一句剧情是**作者的活**。
引擎编一句的下场是屏幕上出现一句世界里没有的话,而它读起来像真的。

它走两条路,**两条都要**:进 `beat_fired` 的载荷(主持人那一屏的回顾读它,
只在开口的那一刻说),也进**那个玩家的叙事流**(一条 `narrative` 事件,
`payload.source == "template"` —— 他随时翻得到)。一次模型都不调。

🔴 **只写得在指着玩家的拍上**(`for_each: {"node": "player"}`)。世界级的拍写它是
**当场拒绝**,不是静默无效:这一句的收件人是「那个人」,而一条世界级的拍没有那个人
—— 写下去它谁也到不了,而**写下去、开得了机、什么都不发生**正是这一层最贵的那种错
(`for_each` 这一格在 3.8.0 上就是那么烂掉的)。
...
⚠️ **「3–5 个选项」是设计意图,不是契约承诺。** 引擎递的是「世界里此刻有多少就多少」,上限是 `host.max_options` —— 一个刚开张、只有一个地点一个人的世界,一屏上就只有一项加自由输入。**它不是 bug**:主持人不会为了凑数编出世界里没有的事。
...
| `deferred_author_sections` | `{段名: 为什么}` | **这一版作者层收不下的段,连理由一起报** —— 和 `deferred_fact_shapes` 逐字同构。✅ **2026-08-31 起它是空的**(`edge` 开了,收件箱 D44);⚠️ **空对象 ≠ 整格缺席**:缺席 = 3.8.0 之前的老引擎(连这个问题都答不出),空 = 这一版一个段都不欠 |
| `edge_author_type` / `edge_author_section` / `authored_edge_keys` / `edge_author_gloss` | `"edge"` / `"edges"` / `["type","from","to"]` / 一段话 | 🆕 **作者层里种得下一条边了**(3.8.0,2026-08-31,§10.16)。探测位和 `beats.author_type` 逐字同构:**这一格缺席 = 这支引擎种不下**,一律 `d.get("plugins", {}).get(…)` |
| `emit_key_requires` | `{"text": ["importance"]}` | 规律那一层的 `emit` 里「写了 A 就必须写 B」的那几对。⚠️ **只管规律那一层** —— 触发器的 `emit` 没有 `importance` 这一格,套过去是假红 |
...
### 10.16 作者层里种得下什么:**实例种得下,边也种得下了**(3.8.0;边这一半 2026-08-31)

创作台做「组织」模板时问的两件事。**(a) 曾经是"不能",2026-08-31 收件箱 D44 开了。**

**(a) 一条边:种得下 —— 作者层第十四个段 `edge`。**
...
补的是一处**不对称**:内核自己的关系图有作者层入口(`relation` 段),而插件的边
没有 —— 于是「门派的初始成员」「谁一开始就收着哪盒磁带」这种**创世态**写不出来,
只能靠运行期动词 `link` 长出来。不开的代价不是"少一个功能",是**把作者往写一件
假东西上推**:配一个只为创世存在的动词让人手点一遍,或者写一个假触发器绕它。

探测位:`contract --json` 的 `plugins.edge_author_type`(= `"edge"`)与
`edge_author_section`(= `"edges"`)。**整格缺席 = 这支引擎种不下**,照老规矩
`d.get("plugins", {}).get("edge_author_type")` 到底 —— 新加的能力格在老引擎上是
**缺席**不是 `null`。同一轮 `deferred_author_sections` 里那条 `edge` 撤掉了,
而那一格**留着并且是空的**:空对象 ≠ 整格缺席。

**三个键,一个不多**(`plugins.authored_edge_keys` = `["type", "from", "to"]`):

- `type` 写**全名** `<插件>.<边名>`,而且必须是这个世界里某个插件真声明过的那一种;
- `from` / `to` 是**节点 id**,形状问 `plugins.edge_node_id_forms`,别照文档抄。
  🔴 玩家是 `agent:player:<id>` 不是 `player:<id>`;`entity:` / `group:` 那两端在
  实例 id 里是**全名**(`menpai.sect:青云门`),不是声明里那个局部名(`sect`)。
- 🔴 **`facts` 有意不收。** 声明过的事实照默认值落地(走的是运行期 `link` 同一条
  路,所以带命名空间:`menpai.辈分`)。写了它**当场被拒**,不是静默丢掉 ——
  理由是运行期那条路上「声明的默认值带命名空间、效果里手写的 `facts` 不带」今天
  就不一致,收作者层这一格等于把一个已经在打架的语义再复制一份。

**三条会开不了机的**:边名/命名空间拼错、两端形状对不上声明、同一份文件里两条边
把一个 `exclusive` 撞了。都和「量名拼错当场开不了机」同一个理由 —— 放行的样子是
那一行安安静静待在库里,规律读不到、提示词里一个字都不出现。

🔴 **出厂插件的边一律拒**(`invitation.*` 那一族):它们是内核**投影的物化视图**
(`rebuild_invitation_edges` 每次开机照事件日志重建一遍),手写一行进去要么下一秒
被抹掉、要么就是**伪造演化态**,而伪造演化态"没有任何地方会报错"。

**只填缺不覆盖,而这一层的粒度是「每一种边」,不是每一条。** 创世照单全种;装进
一个**跑过**的世界时,只种这个世界里还一条都没有的那几种,已经有行的那一种
**整种跳过并点名**。理由:边不进事件日志,引擎手上没有"有人断过它"这份记录,
分不出「还没连」和「连过又断了」—— 而舰队上的世界容器**每次开机都带着
`--world-file`**,逐条补会让每一次重启都把运行期断掉的边接回来。
⚠️ 代价是"改一份文件加第四位创派弟子"做不到(那句 warning 会告诉你走哪条路);
⚠️ 剩下那个角落如实写在这儿:一种边**每一行都被断掉**之后再拿同一份文件开机,
它会**整批重新种下** —— 在"一条都不剩"这一点上,"还没连"和"全断了"在库里是同
一个样子。

**离线两扇门查得动多少,取决于这份文件里有没有那个 `plugin` 记录**:有,就逐条
查到底;没有(一次编辑最常见的形状就是只带几条边),它**明说这一格答不了**,
而不是猜一个答案 —— 开机那一侧拿的是「出厂 + 库里 + 文件」合并后的名单,它答得出。

**运行期那两条来路一个字没变**,而且照旧是加人的正路:给那种边一个带 `link` 的
动词,或者一条订得到的事件 + 触发器。⚠️ **`agent_join` 那条订不到创世那一批** ——
创世时那几条 `agent_join` 发生在插件装上之前,队列里一条都没有(实测:tick 3
之后边表仍是 0 条)。**这一条正是 `edge` 段要补的那个洞**:创世那一批从今天起
直接写下来,不必再去骗一个触发器。

🔴 **仍然别绕道去手写状态层那个 hash**:field 用 `\x00` 分隔、还要顺带写
`edge_types` 索引集合,让消费方各持一份对内部键形状的猜测,正是 `world drop`
当初归引擎的那条理由。作者层这个段就是那条路的正门。
...
- 🔴 **种类上的事实,量名不带命名空间**(`声望`,不是 `menpai.声望`)——
  ...
  🔴 **而"照 `menpai.声望` 写规律那个名字恒等于 0"这句话是错的**(2026-08-27
  更正,逐条敲过):规律**读**它当场三扇门全红(「没声明过这个事实」);规律
  **只写**它,从前是量表里**并排住下两个量**(`声望` 停在默认值、`menpai.声望`
  每 tick 在涨而没有一处读它),`validate` 说绿、零 warning —— **错的不是"那个
  名字等于 0",是那条规律更新了一个没人读的量**。后一半这一轮收了:插件的规律
  写一个顶层 `facts` 里没有的事实,当场拒。
- **要让规律改一个挂在这种东西身上的量,写法只有一种**:声明成**顶层 `facts` +
  `"bearer": "entity:<你的种类>"`**,名字就带上命名空间(`menpai.香火`),
  值照旧住在那个实例自己的量表里。写在 `kinds.<…>.facts` 里的那一族是**裸名**,
  规律写不到它 —— 它适合只读的量(进提示词、给可见性看、被动词的 `set` 改)。

判据 `tests/test_plugins.py::test_插件种类的实例_作者层里种得下_走的就是entities段`。

**(c) 顺带一条会说话的警告**:声明了一种边而这份文件里**没有一个动词或触发器
造得出它、也没有种下过一条**,三扇门都会说一句 —— 两扇离线门进 `warnings`,
开机进 `logger.warning`。**是警告不是错误**(开机是权威,而它收得下),可
**没有一处会说话**才是病:那张表永远 0 条,而作者分不出「造不出来」和「还没有
人连上」。
✅ **2026-08-31 这句话改口了**(上一版这儿就写着"哪天边种得下了它要跟着改口,
否则会对一份写着初始成员的世界报假警报")——「造得出」现在有三条路:`link` 动词、
`link` 触发器、**作者层里种下**。⚠️ `transfer` / `unlink` 照旧不算(它们只搬/断
已有的行,空表上一行都取不到)。
