...
4. **只在五个时刻开口** —— 进地点(`arrive`)/ 新的一天(`new_day`)/ 指着他的剧情拍
   响了(`beat`)/ 他点了「我该干嘛」(`ask`)/ **你回来了**(`return`,3.10.0,2a-②:
   ...
   (`return` > `arrive` > `beat` > `new_day`)—— `return` 排最前是因为一个离线三天的人
