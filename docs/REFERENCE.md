...
| `factions.enabled` | bool | false | 🆕 **3.13.0**(批 3c §2.6):阵营那条 `exclusive` 成员边(`factions.member_of`,玩家 → `group:*`)与两格量(每人 `factions.声望`、世界级 `factions.士气`)。哪几个阵营是**作者写的**;出厂只声明边与量,不声明具体阵营。🔴 **不含计票 / 人数上限 / 禁地 / 全城声望** —— 四样全是聚合或 gates(批 4),`contract.factions.deferred` 点名。**默认关**:一个没写阵营的世界不该多一条它从不使用的边类型 |
