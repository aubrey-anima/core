...
| `World.open(db_path, *, redis=None, world_id="world", …)` | 给了 `redis`,**每个角色的黑板搬进 Redis**,这个进程不再持有它们。那 20 个键(她在哪/在干嘛/饿不饿/打算做什么)此前是纯内存,于是两个进程各开同一个世界文件会读到同一份历史、然后在各自内存里跑出**两个不同的世界**。`world_id` 进键名(一个 Redis 上跑十个世界是常态)。⚠️ 这是第一步不是全部:时钟、在途、当前动作、规划、记忆投影仍在进程里 |
