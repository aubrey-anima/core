...
**没有一处去问在途表**;真去问的是邀请那扇门用的 `World._colocation_gate()`。判据:
...
它给这个理由起了个名字,却从没问过。行首那四个空格是承重的:去掉它,`awk` 会从讲这件事的
docstring 那儿起数。**后果**:她在「咖啡店 → 工作室」的路上、而他在**别处**时,落的是第一格
`elsewhere`,句子里她那个地名是她的**出发地**(`agent_location()` 对在途的人仍报着出发前
那个地名)—— 玩家读到「你在后院,苏晚夏在咖啡店」,而邀请那扇门在同一情形下说的是
「苏晚夏这会儿在路上,还没落脚」。**他在别处时这句读起来完全正常,所以线上不会有人报障** ——
运维碰上"她明明在走路,回执却说她在某个地方",查的就是这一条。修法(改去问
`_colocation_gate` 那四个枚举,别自己按地名猜)已进看板,定版后另开。
...
⚠️ **同一件事在别处还写着好几遍:同的是这三条成因,措辞各文档各写各的 —— 别拿字符串去比。**
另外那几处是 `presence` 命令印在末尾的那一句、答话那扇门的闸表、以及 FOR-STUDIO 里同名的
那一格;读的时候**数它列了几条成因**,别指望逐字对得上。要把闸表那两格取出来看,敲
(答 **2** 行:一行 REFERENCE、一行 FOR-STUDIO;`^\|` 那个锚是承重的 —— 去掉它,下面这行
命令自己就会被数进去):

    git grep -nE '^\| .player_where_unknown' -- docs/

⚠️ **上一版这里写的是「和那两处逐字同一份」,而那句话当场是假的**(第七轮 2026-08-20 写下,
同一轮三个验收员各自独立敲出来:闸表那三个短语在这一段里命中 0/3,FOR-STUDIO 是第三种措辞)。
