...
## 3.22 裁决:图片层(地点 / 背景 / 人物)—— 🔵 **形状已定,代码一个字都还没写**

⚠️ **先说清这一节的性质**,免得你们照着它去改流水线然后撞空:上面每一节都是**已交付**
或**已收到未交付的诉求**,这一节是**第三种** —— 一次跨仓库裁决的落账。它写在这儿是因为
纪律 2(交付要回执)的对偶同样成立:**一个只留在会话里的裁决,下一轮没人知道定过。**
但**引擎这一刻还没有 `location.image`,`contract --json` 里也还没有它** ——
所以你们那条铁律照旧、而且这一次尤其要守:**能力问 `contract --json`,不看这份文档、
更不看版本号**。问不到就是没有。
...
{"kind":"author","type":"location","body":{
    ...
    "image": "https://animametaverse.com/media/……"}}     // ← 新的一格,可选
```

| 那一格 | 规矩 |
|---|---|
| `image` | **带 scheme 的绝对 URI**,和 `portrait` 同一道闸、同一个理由(包是分发物,相对路径发出去就是一张断的图而且不报错) |
| 必填性 | **可选**。它进 `contract --json` 的 `seed.location_optional_keys`,和 `agent_optional_keys` 同构 —— 混进必填集 `location_keys` 等于要求每个世界给每个地方配图 |
...
**二、`data:` 会多一个字节上限,而上限的判决在引擎这边。** 你们那条 `PORTRAIT_MAX_BYTES`
(1 MB)是"早一步的提醒",这是对的;但引擎这一侧**一个字节都没管**过,而这是引擎的漏。
一张 `data:` 立绘会进 Redis 的 card hash、进事件日志(**永久留存、抹不掉**)、进包,
而 `roster()` 每次全量吐 —— 21 个角色各一张,就是每次几十兆。上限进
`contract --json` 的 `character_card` 段,你们照它判,**别再自己钉一个数**。
...
**它走哪条读出口**定(`portrait` 走 `roster`,按需拉;`location.image` 走 `state()`,
网站每几秒轮询一次 —— 后者要紧得多)。

**② 存量世界怎么办 —— 这一条我不拍脑袋。** 加严的第一步是**去查**:现有 `.cyberworld`
与线上那个世界里到底有没有 `data:` 立绘。有 → 上限先落成一轮 warning,下一版才变
error;没有 → 直接 error。这是引擎自己定得了的事,不用惊动任何人,但**得先看一眼真数据**。

**③ "包发给别人自建部署,图就没了" —— 可接受,而且不是格式的缺口。** 那条"图随包走"的路
core 早就留好了,它就叫 `data:` URI;老板选了外链,那是**产品选择要付的代价**。
引擎该做的不是补格式,是让这个代价**看得见**:`world check --json` 会多两段
`external_media` / `inline_media_bytes`,把这份世界文件里所有外链按 host 列出来 ——
...
上传发生在**作业库**那一层(`final_agents.portrait` / `final_locations.image`),
不在打包那一层。连带一条建议:作业库里**把"本地源文件"和"已上传的 URL"分两列存**,
出包只带后者,**有本地路径而没 URL 的当场拒绝出包** —— 引擎那道绝对 URI 的闸在下游
兜着,但等到引擎拒绝时作者已经跑完整条流水线了。离线创作照做不误:没网时只写本地那一列,
要出包时才需要网。

### 还没定的两件(**产品向,归老板,别替他猜**)

1. **「背景」到底是第三样东西吗**,还是"地点图"的另一个说法。三种可能的代价差很多:
   地点图的别名 = 引擎零额外改动;**世界的主视觉**(一张,进 `/play/:id` 才出现)= manifest
   加一格(manifest 有 `extra` 兜着,加可选字段本来就不是破坏性变更);**每个角色一张聊天
   背景** = `CARD_KEYS` 加第四格,和 `portrait` 同一条路。
2. **地点图给一格还是两格**:地图上的**缩略/图标**(小、方)和进到那个地方看到的**场景画**
   (大、宽)是两个用途,一格装不下 —— 界面只好把场景画缩成图标、或者把图标拉成背景,
   两种都难看。建议先一格(语义 = "这个地方的样子",裁剪归界面),但这是可加的。

**在这两件定下来之前,引擎不动 manifest、也不动 `CARD_KEYS`。**

### 契约怎么问(交付之后)

```bash
anima-world contract --json | jq '.seed.location_optional_keys, .character_card'
```

`location_optional_keys` 里有 `"image"` = 这一版带得动地点图;答不出 / 没有这一格 =
**带不动**,你们一个字都别往 `.cyberworld` 里塞(塞进去的下场不是报错,是
`_LOCATION_FIELDS` 那行推导式**静默丢掉**,而世界照跑、日志干净 —— 和角色卡当初
一模一样的坏法)。
