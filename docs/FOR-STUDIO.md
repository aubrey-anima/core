...
| 量 / 规律 / 感知 / autonomy 统计**全都没有 CLI 出口** | 🟡 **四格里两格半已经到了**(2026-08-25 逐条实敲复核,原话写于 1.3.0)。**已到**:`autonomy` 统计 → `anima-world doctor` 那一段(3.6.0);感知 → `anima-world prompt --agent X --json` 的 `perception` 块;**东西身上的**量 → `anima-world ontology --json` 每个实体一格 `values`。**仍然欠着**:① `rule_stats()`(规律这一层跑没跑、写了几笔)**只有 Python 出口**(判据:`git grep -n rule_stats -- anima_world/__main__.py` → 一行不印);② **她身上的**量(`stock:agent:夏` 的 `体力`/`手艺`)一个 CLI 出口都没有 —— 角色不在 `ontology.entities` 里(实敲:`ontology --builtin --json` 的 `entities` 里 kind 只有 `tree`)。⚠️ 第 ② 格对你们最疼:`requires` / `costs` 全写在 `me_*` 上,而"她这会儿到底有多少体力"在 CLI 上问不出来 |
...
| 定时轮次行动率偏稀(约三四天一次) | 🟡 **今天能自己调的那一半照旧**:`autonomy.decide` 是热改模板(`prompt` 那条命令看得见她收到什么),`autonomy.interval_ticks` / `max_per_day` 也是配置。**结构性改法(情境触发,而不是按钟点问)仍然欠着、没有排期**(2026-08-25 复核)。⚠️ 3.6.0 顺手修掉了它最贵的一种浪费:**必然被拒的选项不再摆上菜单**(线上量出来的一轮是 63 次问 / 0 次动作 / 5 次失败,五次全是同一句「这会儿她身边没有人」)—— 那不只是白花一次调用,**她挑了、被拒了,而这次失败教不会她任何事** |
...
🔴 **这一节的存在本身是一条教训。** 能力 **2.0 那一轮就落了**(`3254f36`,**2026-08-05** —— 判据 `git log --date=short --format='%h %ad' -1 3254f36`),
