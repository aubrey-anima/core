...
{"player_id": "u1", …, "memories_redacted": 1,
 "facts": 5,                         // ← 3.8.0 新增:他身上那张量表删了几个量
 "names": 1, "names_skipped": 0, "dry_run": false, "seq": 812, "phase": "done"}
```

- **零也报 0,不许缺席。** 老引擎是整格**没有**(不是 `null`)—— 和 `beats.author_type` /
  `invitations` 逐字同一条纪律,一律 `receipt.get("facts", 0)`。**报 0 是在说"我查过了,
  他身上没有量";缺席是在说"这支引擎不查这个"** —— 两件事,读的人得分得开。
...
| **platform** | 壳**原样转发**回执即可(读门是逐字转发整个 dict)。⚠️ 但这一格**靠重建镜像才到得了线上** —— 在跑的镜像里那一版引擎抹不掉量表,而它抹不掉的时候不报错 |
