...
第 1 条(`importance`、NPC 约玩家)仍然**没有交付**,状态行别提前改。

**所以你们这一侧现在只有一件事要做:检查有没有把联合动词写进排班或行为树,有就挪出来。**
`participants` 的写法本身一个字都不用改。⚠️ 那一版还会带一个**新的可选作者字段**
(能力上的 `importance`,决定"做过这件事旁边的人记不记得")和一扇**NPC 反过来约玩家**
的门 —— 那两样**还没交付**,别提前改状态行;交付时会在这份文档里另起一节,并且
`contract --json` 上会多出对应的一格(**照出口判,别比版本号**)。
