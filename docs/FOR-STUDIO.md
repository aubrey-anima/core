...
| 格 | 值 | 一句话 |
|---|---|---|
| `absent` | `agent` / `player` / `both` / `unknown` | **机器读这一格**。`unknown` = 世界不知道玩家在哪(宿主没调过 `player_move`),和"他站错了地方"是两回事 |
| `gate` | 如 `player_not_here` | 挡下来的那道闸的**名字** |
...
| 查这个世界**有几件长过程白花了时间** | `anima-world doctor`(有就退 1) | ✅ 3.6.0 第三轮(此前只在 `state()` 里,CLI 上没有) |
