...
🔴 **2026-08-25 整表复核。** 这张表里有**四行**把「排 1.4.0」「1.4.0 之后」当排期写 ——
**而 1.4.0 是 2026-08-04 发出去的那一版**,引擎从那以后走的是 2.0 → 3.7.0,
那个里程碑**再也不会到来**。于是这四行的"状态"格说的是一件不可能发生的事,
而读的人会把它当成"排上了、等着就行"。⚠️ **这和「一句说得比做到的宽的话」是同一族**:
它不是假的承诺,是**一个已经不存在的坐标系里的真承诺** —— 更难发现,因为字面上没有一处错。

(⚠️ 表里还有**第五行**也提到 1.4.0 —— `set_rules` 那一行。**它没错**:那一行说的是
"1.4.0 时排过这个名字,而它至今不存在",是**历史引用**不是排期,所以这一轮一个字没动。
**"提到一个死掉的里程碑"和"把它当排期"是两件事**,别一并扫掉。)

**这一轮的改法**:①「排 1.4.0」一律换成**今天的实况**(还欠着 / 已经到了 / 有意不做),
...
| 定时轮次行动率偏稀(约三四天一次) | 🟡 **今天能自己调的那一半照旧**:`autonomy.decide` 是热改模板(`prompt` 那条命令看得见她收到什么),`autonomy.interval_ticks` / `max_per_day` 也是配置。**结构性改法(情境触发,而不是按钟点问)仍然欠着、没有排期**(2026-08-25 复核)。⚠️ **3.2.0**(2026-08-13,`99ed871`)顺手修掉了它最贵的一种浪费:**必然被拒的选项不再摆上菜单**(线上量出来的一轮是 63 次问 / 0 次动作 / 5 次失败,五次全是同一句「这会儿她身边没有人」)—— 那不只是白花一次调用,**她挑了、被拒了,而这次失败教不会她任何事**。判据 `git log --oneline -S'63 次问' -- anima_world/`(只有一条,就是 `99ed871`);机制是 `ToolSpec.requires_present_target` / `requires_target_entity`。🔴 **这半句 2026-08-25 第一版写的是「3.6.0」,错了四个次版本** —— 是照「这件事读起来像 3.6.0 那一单干的」填的版本号,而没去敲那条 `git log`。**在这份文档里填错一个版本号不是小事**:你们按版本号判「我这支引擎有没有这一格」,写宽了就会让一台 3.2.0–3.5.0 的机器被当成没有它 |
