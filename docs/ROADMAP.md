# anima-world 十个大版本路线图(v2.0 – v11.0)

> **⚠️ 这份文档写在 1.0.0 发布之前,它预测的前四版已经落地了。**
> 记忆 2.0、需求系统、经济、社交(原 v2.0–v5.0)**全部并入了 1.0.0 的 db 格式 1**,
> 不再是"未来的大版本"。只当它们的设计存档读,别当路线图读 —— 当前实现以
> [REFERENCE.md](REFERENCE.md) 与 [ARCHITECTURE.md](ARCHITECTURE.md) 为准。
> 真正的未来版本从导演系统(原 v6.0)往后算。

> 规则回顾:**主版本号 = db 格式**。每个大版本都动 db schema,所以每个都是主版本;
> 第二/三位是程序优化。硬钉版模型不变:老世界留在老版本上跑,新世界用新版本生成 ——
> **没有任何一版做跨版本迁移**,这是整个路线图的第一约束。
>
