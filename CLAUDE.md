...
# 最近一次实跑:**1838 passed / 19 skipped**(2026-08-25,`.venv/bin/python`,rc=0)。
# ⚠️ **那一趟之后又加了 1 条**(同日修复轮的判据自测),而**这一行不改成 1839** ——
#   它记的是"我真跑过的那一趟",不是"我算出来应该是几"。我这一轮量的是收集数:
#   `python -m pytest -q --collect-only` → **1858**(那一趟那份代码同一条命令是 1857,
#   `git stash` 前后各敲一次,差 1)。**1858 = 件数 + skip**,和上面那两个数不是同一把尺。
# **秒数这儿有意不填一个数**:同一天在这台机器上跑了四趟,量到 **551 / 445 / 352 / 436 秒**,
# 而最后两趟跑的是**只差几行文档的同一份代码**,件数逐位相同、秒数差 84 秒。理由见下。
# ⚠️ **这一行里的数,只有件数是账。**
