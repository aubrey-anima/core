...
# 🔴 **那 2 条红是 `tests/test_needs_plugin_parity.py`,而它在 `fd1f20d`(本轮开工前
#   那个提交)上一样红 —— 交叉重跑量过,不是推出来的**:
#
#       base(fd1f20d) 9 趟 → 红 5   |   head 9 趟 → 红 9
#       (先各连跑 5 趟,再 base/head 交替跑 4 对,控住"机器此刻忙不忙"这个变量)
#
#   两边都在红,而 head 更稳定地红 —— **两个数之间的差别落在这道闸自己的抖动幅度里**
#   (同一个 base 早一轮是 3 绿 / 5,晚一轮是 3 红 / 4),别拿它当"这一轮改坏了"。
#   ⚠️ 红的那两条是 `test_提示词逐字节相同` 与 **`test_基线自己是可复现的`** ——
#   **后者比的是同一份代码对它自己**,所以它红的时候说的一定不是"谁改坏了什么"。
#   **病根找到了**:那道闸只关了叙事,**关系判定那个线程池还开着**,它的产物落在
#   第几 tick 由这台机器决定。实测把 `scheduler.relationship_judge` 也设成 `None`,
#   那两条 **4/4 全绿** —— 但同一刻**另外两条稳定变红**(`state刨掉…` / `非叙事事件…`),
#   因为关掉判定池**改变了这个世界**,而 `needs_legacy_golden.json` 是**开着它**采的。
#   🔴 **所以这一轮有意没改**:重采基线 = 从**插件路**采一份"旧路基线",而那正是
#   这份文件唯一的用处(旧路删掉之后"从前那个世界长什么样"的唯一证据)。
#   要么重采并接受它降级成"今天的快照",要么想办法把判定池钉成确定性的 ——
#   两条都不是一个写者该单方面拍的,**已上报**。
#   ✅ 顺带:同一天新加的 `test_economy_plugin_parity.py` **没有这个病** ——
#   它采基线之前就把**两个池都关了**(叙事 + 判定),并把理由写在文件里。
...
**`__version__` = 3.8.0(2026-08-26 定版)。world.db 整体退役,世界只住 Redis(+可选 MySQL)。**
