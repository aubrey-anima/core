...
# 最近一次实跑:**1838 passed / 19 skipped,352 秒**(2026-08-25,`.venv/bin/python`,rc=0)。
# ⚠️ **这一行里三个数,只有第一个是账。**
#   · **件数是账**,而且只有一个用处:**别往下掉**。掉了就是有测试悄悄不跑了
#     (collect 报错、夹具坏掉、整个文件被 skip),而屏幕上照样是一片绿点。
#     每加一批测试它就变,所以别拿它判"跑对没跑对"。
#   · **秒数不是账,是那一刻的机器。** 同一份代码、同一条命令,2026-08-25 这一天
#     在这台机器上量到 **551 / 445 / 352 秒** —— 差的是**别人在这台机器上跑什么**。
#     头一趟的前三分之一被另一个会话的一个 13.7 核的活压着,而**屏幕上没有一处会
#     说这件事**:`ps -eo pcpu,pid,args --sort=-pcpu | head` 才看得出来。
