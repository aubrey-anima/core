...
- **契约 72 → 76 格,纯增量**:`edge_author_type` / `edge_author_section` /
  ...
  "这一版不欠段"和"老引擎答不出这个问题"。
- **`engine_min` 仍是 `3.8.0`,没抬到 3.9.0。** 判据是**发出去了没有**,不是"改动大
  不大":`3.8.0` 从没打过 tag、没上过 PyPI、线上也还没有一个 3.8.0 镜像(当天核过
  `git tag` 与索引)。所以"同一个版本号下两支能力不同的引擎"这笔账在这一格上收不到人。
  ⚠️ 哪天 3.8.0 真发出去了,再加段就必须抬版。
