...
⚠️ **而"此刻这份文件里没有 `[Unreleased]`"这句原文 2026-09-02 又不成立了**:
周更链路那一单的裁决开了一个新的。**它是这一段自己批评过的那种句子** ——
一句写着"此刻"而不带日期的实况,会以"现状"的身份被下一个人引用;
现在它带日期了,而带日期的过期实况是**记录**,不是谎话);
...
### 周更链路的裁决(2026-09-02;**只有文档,`anima_world/` 一个字节没动**)

老板 09-02:「我要的是创作者能控制这个东西,相当于每周都能有更新」。core 按总图
「判定程序」两问裁完,结论写进 `docs/FOR-STUDIO.md` §3.62 与总图契约表五行
(全部标着 **裁决,待实现**,目标 **3.10.0** —— 主版本是可挂载性,而这一单一格没动它)。

**这里记的是那一趟量出来的、今天就成立的四条现状**,因为它们各自推翻了一句正在被引用的话:

- 🔴 **把一份内容全落在 `beat` / `config` / `personality`(在册的人)/ `world_setting` /
  `memory`(在册的人)这五段上的编辑包装进一个跑着的世界,世界一个字都不变,
  而 `world check --edit` 答 `loadable: true, errors: []`、`simulate --world-file` 退 0**,
  屏幕上只有关于 `beat` 那一句 warning。**同一个 `not persisted` 门后面站着五个人,
  只有第一个会出声** —— 创作台 2026-09-02 的诉求 C 量到了前三个,后两个是这一趟补上的。
- 🔴 **`config set` 改不了一个正在服务玩家的世界。** `ConfigStore` 开机 `_hydrate` 一次进
  `_cache`,`get()` 只读缓存;实测另一个进程写完之后 Redis 那一行已经变了、`config list`
  显示新值,而跑着的那个进程照答旧值,零报错。**「开关那一格有替代路」这句话对跑着的
  世界不成立。**
- ⚠️ 对照组(同一趟量的,两个方向都有):`:locations` 与 `:prompts`(`world setting --set`
  那扇 3.8.0 开的门)**是跨进程活的**;`:entities` 要**一次 `interact`** 才补课
  —— `_sync_entities()` 全仓只有 `perform_affordance` 一处调,`World.entities()` 这扇
  只读门不补课。
- ⚠️ **`plugin` 段按版本号升级,已经是「同 pack 升级可覆盖自己发过的」这条纪律的先例**
  (实测 `qi 1.0.0 → 1.1.0`,新事实也种下),它不是例外。
