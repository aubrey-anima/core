...
- **一个装过剧情的世界,第二次开机起再也起不来了。** 3.10.0 把「文件里有 `beat`
  ...
  的包」在它眼里长得一模一样**。而舰队每次开机都带 `--world-file`,所以线上龙族
  在 3.10.0 上开不了机(platform 现场撞上,已回滚)。
...
**`storage` 段一个字没动**(实测与 3.9.0 逐字相同,六格)—— `:packs` 不存在,
「装了哪几周」折自事件;运维台那条只比 `.storage` 的 deepEqual **不会红**。
