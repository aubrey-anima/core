...
| 比什么 | 结果 |
|---|---|
| `needs` / `prompt_sha` | **和上一份逐字节相同** |
| `stocks` | 只差那三个 `needs.*` 键 = **搬家本身**(这道闸本来就逐键白名单了它们) |
| `events` | 103 → 99,少的 4 条**点得出名字**:2 条 `memory_seed` + 2 条 `state_change`,全是关系判定那一次的产物(夏/柔在咖啡店那段对话) |
| 凭什么说它出自旧路 | **判据是正的**:`hasattr(anima_world.needs, "factory_plugin")` 在那棵树上是 `False` |
| 上一份没被抹掉 | `fa1507b:tests/data/needs_legacy_golden.json`,sha256 记在新文件头里 |
