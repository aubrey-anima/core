...
`## [3.7.0]` 底下的「发版前那一轮」,于是那一天起这份文件里没有 `[Unreleased]`
—— 上面那句"下面开了一个"讲的是 08-25 到 08-26 之间那一段时间。
⚠️ **那句原文 2026-09-02 上午又不成立了一次**:周更链路那一单的裁决开了一个
新的 `[Unreleased]`,而**当天下午定 3.10.0 时又整段并了进去** —— 这条规矩在一天之内
走完了一整轮(开一个 → 定版并进去),所以此刻这份文件里又没有它了。
**那句原文是这一段自己批评过的那种句子**:一句写着"此刻"而不带日期的实况,
会以"现状"的身份被下一个人引用。现在它带日期了,而带日期的过期实况是**记录**,
不是谎话);
...
`beats._validate_payload` 里写着 `if kind in ("link", "unlink", "transfer")`,
而 **`transfer` 不在 `VALID_OPS` 上** —— 上面那道闭集闸先把它拦下了,
那一支永远到不了。

**一句在代码里写着、却永远不成立的条件,会让下一个人以为剧情拍支持 `transfer`,
然后去查它为什么"不生效"。** 收成两个,并把「要开它得先动闭集与 REFERENCE §9」
写在那儿;闸把这件事钉成一句可验的实话。

⚠️ 同一轮删掉那个测试里的死变量 `head`(注释说「只扫到正文足够远」,
而下一行 `body = text` 扫的是全文 —— **一句和它下一行打架的注释,
比没有注释更坏**,这已经是这一版第三次)。

### 🟢 ③ 旧账:撞出厂 id 那道闸,内容包那扇门单独钉住

3.13.0 那道闸装在 `world_plugin_errors` 上 —— 世界文件 / 开机 / `validate` /
`world check` 四扇门都走它,而**内容包是第五扇**,当时只在别处实测过、没留用例。

⚠️ **「另一条路上也拒」和「这条路上拒」是两件事**:这个仓库为
「新增一种失败,忘了把它接进另一扇门」红过不止一轮 —— 没有这一条,
哪天 `pack install` 换了条校验路,**没有一处会说**。

## [3.13.0] —— 交织与线索(玩法层批 3c) (2026-09-07)

### 🔴 升级前先读:三条不是加法(2026-09-07,验收 B ④)

这一版**绝大部分是纯增**(顶层 18 → 20 段,`.storage` 逐字节不动)。
下面三条不是,**升级前先读**:

1. **`director.first_arc_beats` 从「死键」变成真闸 —— 调过它的世界行为会变。**
   它 3.13.0 前半段就报在 REFERENCE / `config_store` / 契约里,而引擎里
   **它一路只走到一个不读它的形参上**:改成 60 和改成 6 一模一样。
   这一版它真的接到「翻不翻篇」那一处。
   👉 **已经把它调离默认 6 的世界**,升级后头几拍的相位会翻得更快(调大)或
   更慢(调小)。不想变的,把它设回 `6`。**没调过的世界一格不动。**
2. **完整世界的校验多了一种拒绝:effect 指到这个世界没有的角色。**
   3.12.x 上它是绿的 —— 那一层的判断函数当时整个不存在。
   ⚠️ **两扇门的红灯长得不一样,别只认退出码**(A 末轮 ①:上一版这儿写着
   「两扇门从此都退 2」,而那是假的):
   `validate world` **退 2**;`world check` **照旧 rc 0** —— 它答的是
   「我答上来了」,而答案在 `loadable`(见 FOR-STUDIO §3.45,那扇门的退出码
   语义是**有意**和 `validate` 不同的:非零 = 我没答上来)。
   👉 **打的是创作台的打包路**:一份 3.12.x 打得过的世界,3.13.0 可能红。
   红的那几条是真的 —— 那些 effect 从前**运行时静默吞掉**,同意了什么都不发生。
   👉 **闸就写这一行**(FOR-STUDIO §3.45 的原话,抄它别自己发明):

       收得下 = rc === 0 && payload.loadable === true

   ⚠️ **别把 `unchecked_layers` 加进这个 `&&`** —— 那一版 2026-08-27 被 C 视角
   验收 ❌ 掉过:`unchecked_*` 是**账**不是闸,而 `unchecked_state_tables`
   对每一份真实的世界包都非空,拿它当闸会拦下舰队上的每一份包。
3. **`--edit` 那一遍反过来放松了一格:它不问上面那一条。**
   一次编辑手上**没有名册**,在那儿问「这份文件里有没有这个人」必然报红,
   而那份编辑装得进去、开得起来 —— **比开机严是假红,和比它松一样坏。**
   👉 打**只带插件**的编辑时加 `--edit`,effect 指到**目标世界**里已有的人
   是对的写法。

⚠️ 另有一条**闭集缩小**已在 3.13.0 的 ⑧/⑫ 里记过,这里再说一遍免得漏读:
`contract.person_verbs.target_fields` 从六格收到五格(**`who` 没了** ——
没有任何一个 op 拿它当字段名)。照契约那一格读的下游零改;
照文档里抄下来的那份表读的,要重抄一遍。

### 🔴 ① 先立泄漏闸,再开交织 —— 顺序是承重的

任务单 §3.5 那句「先开口子再补闸,中间那段时间里**泄漏是真的发生了,而事件
日志抹不掉**」。所以闸单独一个 commit,那时一条交织的路都还没开。

**那个口子是实测出来的**:一条订 `director_log` 的插件把 `link` 的 `to` 写死成
`player:p2`,边真的连上了 —— 拿 A 的剧情去动 B 的世界,而 `plugin list`
看不出来。⚠️ 裁决原文那条闸写的是「插件试图 **hail** 另一个玩家」,
而量完发现**插件 hail 不了任何人** —— 那条闸会永远绿;打的是真口子。

### 🆕 ② 交织的第一条路径:被撞见

A 当着 B 的面做一件事 → B 那一屏开口,并**指名道姓**;A 在别处做 → B 那一屏
`cached`。**反那一条是承重的**:少了它,「一有人动手就叫醒所有人」也能让正那条
绿 —— 而那不是撞见,是广播。
新增第三张策展表 `CROSSING_EVENT_TYPES`(**有意不复用那两张** —— 三个问题
三张表),投影加一格 `place_move_seq`(按**地点**记,不按人记),
时刻钥匙加第四格 `here_seq`。
🔴 **两条线仍然各是各的**:B 的屏上可以有「楚子航端详了老橡树」,
但不许有 A 的 `promise`/`why`/张力/线 —— 有闸钉着。

### 🔴 ③ 对人动词「同意了也什么都不发生」(tool 诉求 E,真发包到龙族时量出来的)

**两半,而第二半和占位符无关:**

1. **没有占位符指得到「你刚才问的那个人」。** 一条对人动词的全部意义就是
   **对谁做在运行时才知道**,而作者只能写死一个角色 id。tool 写的
   `{"as": "$target"}` 引擎**照收**(`world check` rc 0),运行时展开不出、
   被 `except` 吞掉,玩家侧 `ok:true / accepted` 一切正常。
   现在 `$target` 是契约里的占位名(`contract.person_verbs.target_placeholder`),
   拼错或指到一个不存在的角色**加载期当场拒**。
   **一个装得进去、却什么都不做的动词,比一个装不进去的动词坏得多。**
2. 🔴 **那几行从来没发过事件** —— 展开完只把 op 名字攒进 `landed` 就完了,
   **一条都没落库**。即便把角色 id 写死也一样什么都不动。
   **占位符只是这条 bug 的一半**,而这一半是真正让龙族那三个动词
   「同意了世界一格不动」的那一半。
   ⚠️ 展开不出现在也**吭声**(上一版静默 `continue`,日志上也看不见)。

`plugin list` 补一格「对人动词 N」——那行 `动词 N` 是 affordance 那一族,
而对人动词永不走 affordance,于是装了三条的插件在那屏上显示「动词 0」。

### 🟡 ⑥ 真站第七轮剩下的:空话的敲门,和两个人撞进同一条线

- 🔴 **一条一个字都没有的敲门**(C ③):`/contacts[0]` 那条 `text` 是空串。
  三条发射点**三种空法**:她闲着想起你那条**根本不填 `line`**、编剧那条是
  「没写台词就不带这一格」、约好回话那条同样没有。
  **一条「有人找过你」却一个字都没有的敲门,和没有那条敲门是同一件事** ——
  而它还**占掉了她今天那一次开口的额度**(`claim_hail`)。
  兜底句一处生成、三条共用、按次数轮着说(不掷骰子:同一份日志重放两遍要得到
  同一句;而一句永远不变的台词会让她显得像个推送)。
- 🟡 **两个玩家撞进同一条线**(C ④):同对手、同相位、同 `story_text`,
  只差措辞。`select_cast` 加**第六道软闸** —— 别人那条开着的线上的对手让位。
  🔴 **换的是对手,不是剧本**(裁决 §2.12):C 提的另一条修法是「把别人的线
  摘要放进提示词」,而**那和 §2.2 那条红线打架** —— `promise` 是别人剧情的
  原文,进了提示词模型完全可能抄进 `line`,**从这个人的屏上漏出去**;
  「摘要、不带 id」不够,**泄漏的是内容不是 id**。
  对手是引擎挑的,而"谁被占着"**不含任何别人的剧情内容**。
  ⚠️ 两道软闸叠起来最容易把 cast 筛空,而**空 cast 的下场是沉默** ——
  比撞线坏得多,所以那条「筛空了整份还回去」连着这道新闸一起验。

### 🔴 ⑬ 验收 A(整体 release):**线索与阵营,作者层一条都点不亮**

两个新玩法交出去了,而**没有一条真路走得通**。这一节按 A 逐条记。

- 🔴 **玩家在图上有两个形状,而写的人和读的人各写各的**(A ②)。
  边**属于** `agent:player:<id>`(`plugins.EDGE_NODE_ID_FORMS["player"]`,
  那一行的注释里就写着「这一行最容易写错」),而关系 / 账本 / 库存 /
  事件顶层的 `who` / 在场位置一律是 `player:<id>`。
  `_apply_verb_edges` 写前者、`player_clues`/`player_faction` 读后者 ——
  **`link` 返回 True、日志一条不少,板子上是 0。**
  ⚠️ 而夹具当时直接调 `apply_edge_effect` 传了个 `"player:p1"` 字面量,
  于是**它和读的那一半错成了同一个样子**:一条不红。
  修法**不是在两处各转一次**,是把规范化收进 `Scheduler._edge_node`(唯一汇合处)
  —— 每个调用方各记一遍的下场是:哪条路忘了转,那条路上的边就安静地连到
  另一个节点上。同一个 `player_faction` 里「声望格用 `stock_owner_of`、
  边格没用」那一处一格之内的分裂也一并收掉。
- 🔴 **解锁路一(`verb_effect`)从来没通过**(A ①)。规矩是「只连得动自己声明的
  边」,而 `clues.knows` 是**引擎出厂的** `clues` 插件声明的 —— 于是
  **没有任何一个作者连得动它**,而 FOR-STUDIO §3.71(c) 正照着这条路发给了 tool。
  新增 `contract.plugins.shared_edge_types`(从出厂模块自己数出来,不手抄):
  出厂插件的那几条边**任何插件都写得下**。
  🔴 **公共边和「别人的边」不是一回事**:出厂插件是引擎的一部分,
  它声明那条边**正是为了让作者去连**;而另一个作者插件的边照旧归它自己管
  (那一条一个字没松,闸一并写了 —— 只写正面那条的话,把闸整个删掉也是绿的)。
- 🔴 **解锁路二(`beat_op`)在契约里,而引擎里一行实现都没有**(A ①)。
  `link` 不在 `beats.VALID_OPS` 上 —— 一份这么写的世界**开机当场拒**。
  新增剧情拍的 `link` / `unlink` op:「这一拍响了,他就知道了这件事」。
  🔴 **一条新的「写世界」的路都不开** —— 走的是内核那份 `apply_edge_effect`
  (约束、`exclusive`、边上事实的默认值全在那儿查一遍),剧情拍只是又一个调用方;
  **也不发事件**(边本身就是状态,为它再造一种事件就是第二份真相)。
  ⚠️ **没连成就不进 `ops_applied`**:一个"什么都没做"的 `link` 和一个"建成了"的
  在日志上长得一样,而那一格是作者唯一读得到的回执。
- 🔴 **解锁路三(`director_reveal`)撤了,连理由一起写进契约**
  (`contract.clues.refused`)。它不是"没来得及做":**编剧是个模型**,
  要它揭哪一条就得把线索名单放进提示词 —— 而未解锁线索的名字**就是它的谜面**,
  模型抄进 `line` 就直接印在玩家屏幕上。和 §2.2 堵的那种漏是同一种。
  于是 `unlock_paths` 三 → 二,**而收的这一版才是真的**:
  上一版那三条**一条都连不上**,tool 正照着它写第 3 周的底稿。
  **一个报得出、却走不通的取值,比不报它更坏。**
- 🔴 **`apply_edge_effect` 对谁都没声明过的边类型照连**(A ③)。
  一个拼错的 `type`、一个开关关着的出厂插件、一条早就删掉的边 ——
  全都**安静地建成一条边**,`link` 返回 True,而没有任何一处读得到它
  (读的人按声明去读)。**一条谁都不认识的边,和一条没连上的边,
  在库里长得一模一样。** 现在当场拒并留一句说得出理由的 warning。
  ⚠️ 它还骗过了这个仓库自己的夹具:线索/阵营那两个文件当时**直接调这个函数**,
  于是 `clues.enabled=False`(根本没有这种边)那一趟也是 True ——
  **连插件装没装上都没验到**,而那两条用例的名字叫「三条路都连得上」。
  两个夹具改成走真门(作者插件 → 玩家做动词 → 板子看得见)。
- 🔴 **那道新闸只查边类型,不查两端**(A 末轮 ③)。一条拍里写
  `{"op":"link","type":"clues.knows","from":"阿岚"}`(这条边的起点声明的是
  `player`)**建得出来**:`ops_applied` 里有它、库里真有那一行 ——
  而板子读的是 `agent:player:<id>`,**那条边谁也读不到、谁也看不见**。
  **回执说成了,而世界里什么都没发生**,这是新面上的第二处撒谎。
  `link` 那一刻按声明的两端查形状,判断和作者层 `edge` 段那道闸**是同一个函数**。
  ⚠️ 多认一种情况:**借来的种类** —— `entity:<kind>` 那个种类可能是插件自己
  声明的(实例 `<plugin>.<kind>:<名>`),也可能是**作者写的**(实例 `<kind>:<名>`);
  出厂的 `clues` 指着作者写的 `clue`,正是后者。**只认前一种的话,
  这条出厂边一条都连不上** —— 拦过头和漏掉一样坏,只是方向相反,两条都上了闸。
  ⚠️ 顺带修掉一处**从来没生效过的声明**:`factions.member_of` 的终点写着
  `group:*`,而端点那一层没有通配符 —— 它把 `*` 读成种类名。
  它一直没被发现,正是因为 `link` 那一刻从前根本不查两端。
  ⚠️ **而那道端点闸自己也只装在一支上**(A 四轮 ②):`transfer` 整个绕过它 ——
  一条 transfer 到「老橡树的来历」(少了 `clue:` 前缀、根本不是实体)答
  `ok: True`、库里真落一行,**而板子把那个 id 原样印出来**;
  同形状的 `link` 却拒。**同一个形状两支两个答案,正是这一族的病根。**
  端点检查提到分支之前,`link`/`transfer` 共用(`unlink` 有意不查:
  它只删不写,形状不对的 id 一条也匹配不到,拿它去拒反而挡住
  「把这一端上所有边断掉」那种只给一端的正当写法)。
  ⚠️ 顺带收紧「借来的种类」那条回落:它是给**借来的**种类开的口子,
  不许拿它放行「插件自己声明的种类漏了命名空间」——
  `menpai` 声明了 `sect`,而 `sect:青云门` 那个 id **一条实例都没有**。
  ⚠️ 再顺带:`group:*` 那个从来没生效过的写法在三处留了镜像
  (`config_store` 的说明 —— 它经 `contract.config` 直接发给下游、
  FOR-STUDIO §3.72(a)、REFERENCE 配置行),三处一起改成现行声明并上一道闸
  (从真声明里读那两端的形状,三处都得对得上)。
  ⚠️ **同一批还照出了第二处**:§2.2 那个夹具的 `mine.记住` 两端声明成 `agent`,
  而它订的是 `director_log`(玩家面事件)—— `self` 与 `event.who` 都是**玩家**。
  声明和它真连的东西对不上,而那条边**谁也读不到**,用例照绿。
  **一道新闸的第一份价值,是把既有的假绿照出来。**
- 🟡 **放开公共边的那一下,顺手把一条产品裁决也放开了**(A 末轮 ④,调度台裁)。
  `factions.gloss` 头一句写着「站了就回不去」,而作者写一条 `unlink` 就能叛出改投
  —— **一条动词不该推翻一条产品裁决**(3c §2.10 ④)。
  ...
  **关掉一条路而不说去哪儿找,下一个人会以为是漏了**。
  ...
  `shared_edge_ops` 保证没有第二条路把第一条边摘掉。
- 🟡 **泄漏闸没覆盖新门**(A ④)。§2.2 那条判据的原话是
  「**新加一扇只读门忘了它 → 闸红**」,而闸里手写着 `host_turn` /
  `player_story` 两扇 —— **同一批新开的 `player_clues` / `player_faction`
  一扇都没被扫到**,判据不成立。
  门表改成从 `World` 自己数出来(公开、只吃一个 `player_id` 的方法),
  **加一扇门自动进**;例外表(三个写门)逐条写理由,**而且它自己有闸** ——
  一张没人验的例外表是这条闸唯一的出口,往里加一行就能让任何一扇门不被扫。
  试牙:临时加一扇漏别人 `promise` 的只读门 → 当场红,而且**报得出是哪扇门**。
- 🟢 **三条小的**(A ⑤)。① `clues.py` 那句「一个 `text` 都没有」和它**下一行**
  打架(`BOARD_KEYS` 末项就是 `text`)—— 说清楚那个 `text` 是**板子自己那句人话**,
  不是任何一条线索的正文;**一句和它下一行打架的注释,比没有注释更坏**。
  ② `options_field` 在两处是两个意思:`person_verbs` 那段指的是
  `player_options()` 返回里的**那一格**,而 `clues`/`factions` 写的
  `"clues"`/`"faction"` **不是任何一格**(照它去取拿到 `None`)。
  定死一个意思、把那两格**删掉**(那两扇门返回的就是板子本身),并上闸:
  **声明了它,真门返回里就必须有那一格**。
  ③ **作者插件的 id 撞上出厂插件,现在当场拒**:插件 id 就是命名空间,
  同 id 是两份对同一批键的声明 —— 开 `clues.enabled` 那一刻作者那份被换掉、
  关掉又把两份一起摘走,**他写的东西不见了而没有一处报错**。
  ⚠️ 这条**不能靠"隔离"解决**:键名空间是共用的,隔离就得给每个键再加一层前缀,
  而那会改掉每一个已经发出去的键名。

### 🟡 ⑫ 验收 B:**回执那一侧的表是裸的**

`docs/FOR-STUDIO.md` 是**发给创作台的那一份** —— tool 照它填 effects、
照它判「哪些动作会被别人看见」,而它的每一张表都是**手抄**的,一道闸都没有。
实测:把 `target_fields` 改回六格(带上 3.13.0 已经删掉的 `who`),
**77 条用例一条不红**;撞见那五格、以及「一屏最多(今天 6)」那个数,同理。

这个仓库为「一份没人验的键表」红过三轮(REFERENCE §4.8 那句「顶层 N 段」
在 13 上停过一版),而**那三轮修的都是 REFERENCE,回执这一侧一直没人管** ——
**发给别人的那一份,比留给自己的那一份更该有闸。**

新 `tests/test_for_studio_tables.py`:两张表按名字**逐格**比
(不比条数 —— 只比条数的话,加一格的同时漏掉另一格仍然是绿的),
另加 `crossing_state_kinds` 收窄到哪一支、以及 `crossing_limit` 那个数。
🔴 **而这一批自己又抄了四张裸表**(A 末轮 ②):`unlock_paths`(刚从三条收到
两条)、`shared_edge_types`、出厂插件 id 名单、玩家节点那个形状 ——
99 条用例一条不红。**加一张手抄表和给它上闸,是同一次改动的两半;
只做前一半的那一版,下一轮验收必然还站在这儿。** 四张各补一道。
⚠️ 定位靠段落标题与反引号里的字段名,**不靠行号、不靠表里的内容** ——
一道靠会变的东西定位的闸,每次真该它响的时候都先自己瞎掉。

### 🔴 ⑪ 验收 A 第三轮:一个**死键**,和一道**比开机门还严的假红**

- 🔴 **`director.first_arc_beats` 一路没接上**(A ①)。它在 `_director_beat` 里
  被读出来,喂给 `pick_move` 一个 **它根本不读的形参**;而真压低目标的
  `phase_target` 只由 `next_phase` 调,`next_phase` 的签名里**没有 `first_arc`**
  —— 永远吃常量 6。**把这个键改成 60 和改成 6,世界里一个字都不差**,
  而 REFERENCE / `config_store` / CHANGELOG / 契约四处都把它当可调闸报给了下游。
  这正是 ⑤ 那句话反过来咬人:**一句已经发给下游的承诺,兑现不了就是假承诺。**
  修法:`first_arc` 一路传到 `next_phase`/`phase_target`,而
  `pick_move` **连形参都不收** —— **一个收下来却不读的形参,
  会让调用方以为自己已经把那件事说出去了**。
  ⚠️ 顺带换掉一道**恒等式闸**:`test_压低目标只影响翻篇_不影响写多大` 原本拿
...
- 🔴 **`world check --edit` / `validate world --edit` 假红**(A ②)。
  ...
  又是从**这份文件自己的** `agents` 里数出来的。于是一份只带插件的编辑,
  effect 指到目标世界里**明明躺着的**那个人,被两扇门报成「指不到」;
  **开机门照收,完整世界那两门答 `[]`** —— 三个答案两种,校验器是错的那一个。
  撞的正是这个仓库的老纪律:**比开机严是假红,比它松是假绿,
  两种都比没有校验器更坏。**
  修法:凡是**要看别的段**的检查器,搬进新的一张表 `COMPLETE_WORLD_CHECKS`
  (只在 `complete=True` 上跑),`world_plugin_errors` 那句「没有跨引用」
  于是重新变成实话。⚠️ 两条用例一起才够:只写「编辑不报红」的话,
  **把这道闸整个删掉也是绿的**。
- 🟡 **「三条发射点」那条闸只走到两条**(A ③)。`_fire_due_followups`
  (她说过的「等会儿再说」到点了)那一条**夹具从来没触发过** ——
  把那一格 `line` 整个删掉,44 条用例一条都不红。
  **一条名字里写着「三条」的闸,只扫到两条,和没扫到那一条是同一件事。**
  夹具现在真的排一条到期的约,并且**先断言 `reason` 里有 `delayed_reply`**:
  少了这一句,哪天那条路自己不响了,闸又会安静地退回两条。
- 🟡 **删掉三个死格那一批,一条闸都没留**(A ④)。把 `who` 加回
  `TARGET_FIELDS`、把 `"spent"` 加回 `_outcome_moved`,全仓一条不红 ——
  **一次靠读代码读出来的删除,下一个人靠读代码是读不回来的。**
  现在两条判据都写成机器问得出的话:`TARGET_FIELDS` 里的每一格
  **必须真有 op 拿它当字段名**(表就是 `beats.OP_REQUIRED_FIELDS`);
  `_outcome_moved` 那条改问行为 —— 一个只扣体力、不改目标的动词**是动了东西的**。
  ⚠️ 顺带补一处**回执撒谎**:FOR-STUDIO 那行 `target_fields` 还列着 `who`,
  而库里已经没有它了 —— **发出去的那句话没跟着改,等于那一改没做完。**
- 🟡 **`_threads_taken_by_others` 会报重**(A ⑤):几条线开在同一个人身上时
  回几份同一个 id。下游只当集合用,所以那不是一个**错的**答案 ——
  **但它是一个会变长的答案**:哪天有人拿它 `len()` 去说「有几条线占着人」,
  那个数是错的,而**那一刻不会有人想到问这儿**。去重 + 排序(确定)。
  ⚠️ **小池子上这道软闸会被自己的软性吃掉**(三角色的世界,一个被硬闸筛掉,
  `recent` + `taken` 一起筛空 → 整份还回去 → 两个玩家又同一个人)——
  **兜底那一条是有意保留的**:「宁可撞脸,不可沉默」。
- 🟢 **两道软闸改成分级退让**(A ⑤ 的后半,调度台裁:做)。
  先两道都上;挤不下就**先丢 `recent`、还保着 `taken`**;都筛空了才整份还回去。
  **次序的理由是这两道闸不一样重**:`taken` 守的是产品命题
  (两个玩家走出两条不一样的线),`recent` 守的是同一个玩家眼里的新鲜感 ——
  **挤不下的时候先丢新鲜感**。⚠️ 最后那一步一个字没改:空 cast 的下场是沉默。

### 🆕 ⑩ 阵营(批 3c §2.6):今天全有的四样,和有意不做的四样

出厂插件 `factions`(开关 `factions.enabled`,**默认关**)只声明
**一条 `exclusive` 成员边**(`factions.member_of`,玩家 → `group:*`)
和**两格量**(每人 `factions.声望`、世界级 `factions.士气`)。
哪几个阵营是**作者写的**(`group:狮心会`)—— 和线索那一层逐字同一条分工。
新门 `World.player_faction(player_id)` → `{faction, faction_name, standing,
standing_text, text}`(`contract.factions.options_keys` 为准)。

- 🔴 **「站了就回不去」是内核在 `link` 那一刻查的,不是插件自己判。**
  `exclusive` 不是一句建议,而**放行它的样子是安静的**:两条 `member_of`
  同时挂着,`plugin list` 看不出来,而提示词里他同时属于两个阵营。
  闸真的站第二边一次,断言**内核拒**(试牙:`exclusive` 翻成 `False` → 红)。
- 🔴 **计票 / 人数上限 / 禁地 / 全城声望,一行都没有** ——
  四样全是**聚合或 gates**,而那是批 4;`contract.factions.deferred` 把它们
  **点名**,免得创作台等一个不会来的东西。
  **在一个没有聚合的引擎上手写一遍计票,就是把批 4 那件事做进一个插件里**,
  而那份实现以后要被删掉重写;在被删掉之前它是**第二份真相**
  (引擎算一遍、插件算一遍,而两边不一致时没有一处会报错)。
  ⚠️ 这条闸拦的是**写这段代码的人自己**:计票"顺手写一下"很便宜。
- 声望的分档词(`生面孔 / 有人记得你 / 说得上话 / 一句顶十句`)
  **只有一份表**,契约那一格照着它生成 —— 和 `tension_text` 逐字同一条:
  各译一遍的话,同一个数会在引擎屏 / 创作台 / 站点上说三种话。

### 🆕 ⑨ 线索(批 3c §2.4):零新原语,而三条纪律各有一道闸

一条线索 = 一个实体;「他知不知道」= 一条边(`clues.knows`)——
`kind` / 实例 / 边 / 可见性,四样今天全有。出厂插件 `clues`(开关默认关)
**只声明那条边,不声明 `clue` 那个种类**:线索长什么样是作者的事。

三条纪律,每条都可验:

1. 🔴 **只报存在与解锁状态,不报内容。** 已知的给 id,**未知的连 id 都不给**
   —— 一个未解锁线索的 id 往往**就是它的谜面**。
   闸拿一个哨兵字符串扫**五扇只读门**(不是"看看有没有 text 字段":
   内容可能从任何一格漏出去,而漏了的那一格不会报错)。
2. 🔴 **解锁没有第四条路**(`contract.clues.unlock_paths` 点名三条)——
   **别等一个「订 `conversation` 的作者判定」**,那要判定运行器(批 4)。
   和「作者层写不了 `confront`」同一课:**一个等不来的东西会让人一直等着,
   而不是换一个写得出来的写法。**
3. ⚠️ **不报「怀疑」那一档** —— 它要置信度,而那是判定那一族的东西。
   **报一个算不出来的档,就是让屏幕替引擎撒谎。**

🔴 **而这一段自己犯了它讲的那件事**:`ec4e13e`(线索那一 commit)**带着一条红提交**
—— REFERENCE §4.8 那句「顶层**N**段」还停在 `十八`,而真门已经答 19 段,
`test_REFERENCE_里那份顶层段清单_和真门逐格相等` 当场红。
阵营这一 commit 一起补上(`十八` → `二十`,清单里补 `clues` / `factions`)。
**教训不是"跑测试"**(这道闸 3.10.0 就有,而且正是为这句话建的),
是:**加一个顶层段,那句话就是这次改动的一部分,不是"文档回头再说"。**

### 🔴 ⑧ 验收 A 第二轮:三条 🔴 **全是我上一轮的修法本身修过头 / 没修全**

13 条自报试牙条条有牙、全量对得上,而**修法自己错了** —— 这一批的教训比上一批具体:

- 🔴 **收窄一条判据时,要问它原来收的是哪几样**(A ①)。「旁观者不吃编剧那一拍」
  是对的,而我的落法是把 `(acted_since or recap)` **整个砍成 `acted_since`** ——
  `recap` 从 3.12.1 就在,收的**不只是撞见**:「一拍为他响了、而他没动手」
  从此**编剧一个字不写**。现在撞见那几行单独一份,守卫只看他自己那几条。
- 🔴 **「不调模型」那句话要全时成立**(A ②)。我为了不吞掉撞见,把
  `only_refused` 直接翻成 `False` —— 于是**屋里有人动一下,白按屏就变成
  `source=mock` 走模型路,而「你为什么白按」一字不剩**。
  改成:照旧走模板路,而模板路带上 `recap`。
- 🔴 **「三处并一处」并的是产地,而读它的两处也得给同一个参数**(A ③):
  `recap_lines` 那处调 `engage_line` **没传 `changed=`** —— 同一件事,
  回执说「什么都还没动」、屏上说「这得花上一会儿」。
- 🟡 **一张「哪几种事看得见」的表,收了印不出字的事件,就是在白叫醒人**(A ④):
  整个 `state_change` 进表,而只有 `location_join` 印得出字 ——
  一条关系变动把旁观者的屏白叫醒(屏重开了,撞见那几行是空的)。
  收成 `crosses_here()` **一处判断两个读者**(投影那格与印字那处)。
- 🔴 **一句已经发给下游的承诺,兑现不了就是假承诺**(A ⑤):
  `effect_errors(known_agents=…)` **全仓零生产调用方** ——
  「指到不存在的角色当场拒」只活在 CHANGELOG 和 FOR-STUDIO 里,
  而 `world check` 照答 `[]`,**而 tool 正照着它填 effects**。接上线。
- 🟢 撞见那张表、`crossing_limit` 与截断那句话进契约与 FOR-STUDIO(A ⑥)。
- 🟢 三个死格(A ⑦):`target_fields` 里的 `who` **没有任何 op 用这个字段名**、
  `_outcome_moved` 查了一个 `_spent` 不产出的 `"spent"`(**查一个不存在的键
  不会报错,只会永远答 False**)、取名那处取自筛前的行。

### 🟡 ⑦ 新玩家头一小时到不了摊牌(真站第七轮 ⑤)

真站实测:13 条 `director_log` 里 `move` 只有 `collect` 9 / `approach` 3 /
`breathe` 1,**`confront` / `reward` / `callback` 与 `roll` 零触发**,张力 0.05。
**3b 那整套玩家面,对一个新玩家等于不存在。**

`confront` 要 `phase == "climax"`,而 climax 要张力先攒过 escalation 的目标 —— 
在 `ceiling` 0.6 的世界里那是四五拍开外的事,而每小时只有 6 拍。
新增 `director.first_arc_beats`(默认 6):头几拍**把相位的目标压低**,
让这条弧在第一小时内走到第一个节骨眼。实测头 3 拍就到得了 `confront`。

🔴 **而第一版我把这个杠杆用反了,值得记**:我把 `pick_move` 的目标也压低了 ——
而 `gap = 目标 − 张力`,**目标压低 = gap 变小 = 动作变小**,
`confront` 恰恰要 `gap >= 0.30`。于是「想让新玩家早点摊牌」那一改
**恰好把摊牌关掉了**(`test_摊牌只在节骨眼上` 当场红:climax 上拿到 `reveal`)。
**压低目标只该让相位翻得快,不该让动作变小** —— 两个读者,一处压、一处不压。

### 🔴 ⑤ 验收 A 对 3c 第一批:九条,而其中三条是**我修完之后新造的**

- 🔴 **把一格从别人的桶里搬出来,就得给它自己的桶**(A ①)。我把降级从
  `refused_by` 搬进 `downgraded_from`,而搬完之后它**三个桶一个都不进**:
  `doctor` 上彻底看不见,`downgraded_from` **全仓零消费方** ——
  那次搬家只是把它藏得更深。`doctor` 补一个降级桶。
- 🔴 **撞见把「他不在场时那儿发生的事」也报给他**(A ②):只比地点、不比在场,
  于是 B 走进咖啡店的第一屏就读到他没看见的事。
  **那个函数自己的 docstring 写着「撞见的语义是当面」,而代码没照做** ——
  一句写在 docstring 里、代码没做的话,比没有那句话更坏。
  改成按**事件那一刻**他在不在场算(位置从同一条日志折出来,不另存表)。
- 🔴 **撞见行不去重也不截断**(A ③):一屏 20 行同一句进提示词也进屏。
  去重 + 截到 6 + **截了吭声**。
- 🆕 **走进来也算撞见**(A ④,调度台口径):`travel` 的 `loc` 是**出发地** ——
  只认它的话**到站那一下没人看见**;`state_change{location_join}` 进表。
- 🔴 **同地那道闸没有自己的用例**(A ⑤):拆掉它官方 139 条全绿。补反用例。
- 🔴 **白按那一屏把撞见吞掉**(A ⑥):模板路 `recap=[]` 而窗口已推过,
  下一屏 `cached`,那一行**永远丢**。
- 🆕 **旁观者不吃编剧那一拍**(A ⑦,调度台口径):撞见行是引擎合成句,
  不该触发 LLM 为旁观者写拍 —— 同地 n 个旁观者 = 每次操作 n 次调用,
  而**撞见不是剧情**;把它算进「他动了」就是悄悄升级成共享剧情。
- ⚠️ **我自报的 A① 试牙是假的**(A ⑧):我"改回"的那行不是修改前的代码,
  而新 `parse_decision` 根本不发 `refused_by` —— 那一改是**恒等式**,
  用例当然照绿。真正有牙的是 `director.py` 那半。**回报里改正了。**
- ⚪ 每屏两次全量回扫合成一次;取名只取**真出现在那几行里**的人(A ⑨)。

### 🔴 ④ 真站第七轮:一句说大了的话,和一份没被抹掉的 GM 笔记

- 🔴 **零效果的长动词说成「你开始拉票狮心会了。」**(C ①)。龙族那三个
  (报到 / 拉票 / 训练)是 `started:true occupies:true` 的**长动词**,
  回执 `changed={} me_delta={}` —— 起了头,而且这一下什么都没花掉。
  而 3.12.1 补的「你试着…没什么变化」那道守卫**只挂在瞬时动词那条路上**。
  🔴 更要紧的是:屏上那句**既不是 `engage_line` 也不是 `interaction_line`**
  —— 它是 `scheduler._player_said` 里的**第三处产地**,
  **同一件事,回执和屏上两种说法**。三处并成一个函数族,长动词也分两句。
  ⚠️ 只扣体力、不改目标的动词照旧**不算**零效果(3.12.1 那条规矩)。
- 🔴 **抹除扫不到编剧那几格**(C ②,合规)。`director_log` 的
  `why` / `line` / `promise` **一个字没动** —— 一份已注销账号的
  **GM 笔记与剧情原文原样留在库里**,而回执上没有一格提到它。
  ⚠️ 对照组是承重的:`host_scene.text` **早就被抹了** —— 所以这不是
  「抹除整个没跑」,而是**那张表少了几格**;一处抹了、一处没抹,最难查。
  `_ERASE_TEXT_KEYS` 补 `why` / `line` / `promise` / `stake_text` /
  `outcome_text` / `said`。
  🔴 **而「`player_id` 出现 0 次」那一半没做,理由在回报里** ——
  `erase_player` 的 docstring 明写着「**不透明 id 保留,不换假名**」,
  那是被跨进程折叠否决过一次的设计(假名映射一旦落库就等于没抹)。
  **这条要改,得先推翻那个决定**,不该由一次修 bug 顺手做掉。
...
3b 第一版把 `confront` 的输赢、`reward` 还了哪条线、`callback` 怎么收的
**只写进了 `director_log`** —— 而那是 **GM 笔记**(`why` 就是写给创作者的
那一句)。玩家**没有任何来源**读得到「我押了什么、输赢了什么」,
而老板那句「剧情要有张力」的可验形式正是**玩家必须看见赌注和结果**。

照 `roll` 那条已经走通的路补三条**玩家面事件**:`confront_settled` /
`reward_settled` / `callback_settled`,载荷自带人话(`stake_text` /
`outcome_text`)—— 站点按 `recent_events` 接,**不自己译**。
`player_story()` 另加 `closed_threads`(每条带 `outcome_text`):
只给开着的那几条,玩家就永远读不到自己那条线的结局。
⚠️ **有意不进 `SUBSCRIBABLE_EVENTS`** —— 那张表是**插件触发器**的闭集
...
两次漏发都出在那儿:引擎加了一格、手抄那份没跟上,**那一格永远送不到消费方,
而两边都不报错**。现在契约报 `{事件类型: [键…]}`,照它生成。
⚠️ 判据是**包含**不是相等(`state_change` 按 `kind` 分支带格)。
闸(`tests/test_event_payload_keys.py`)装上当场就逮出**我自己那张表漏的六格**
(`state_change` 的 `as_name` / `axes` / `delta` / …)—— 它第一天就还了本。

### 真站第六轮那两条

- 🔴 **同一 tick 内动词 accepted 之后仍 `cached`** —— **本地复现不出来**:
  短动词每次都发 `entity_interaction`,五次五屏(用例已加,时钟一格不推)。
  留一问给 platform:那三发 `/tool` 的响应体里 `ok` 到底是 `true` 还是 `false`
  —— **`200` 只说明这次调用没抛,不说明那个动作做成了**;若是 `false`,
  它走的就是本版新加的「被拒也开口」那条路(`source: "template"`)。
- 🟡 **古德里安把玩家叫成路明非。** 根不在模型:那份提示里**根本没有玩家的
  名字**,只有一串「你这一拍能派的人」—— 要它写一句"对他说的话",
  而手边唯一的人名是那张名单,**它只能从那儿抄一个**。
  现在提示词带上玩家的称呼(走 `_relation_name`,和记忆、关系事件里印的
  **同一个**),并写死一句「那张名单是你能派谁,**一个字都别拿来称呼他**」;
  没名字时说「你」,**不编一个**。

### Fixed(3.12.0 首批的其余几条)

- 🟠 **`install_authored_pack` 传了 `on_boot=True`,而那条路是 `pack install`**
  —— 和 `_plugin_bodies` 自己的 docstring 正相反。
  ⚠️ **它今天没有可观察的症状**(更上游还有一道硬预检先拦下降级),
  所以这一条修的是**意图和代码对不上**,不是一个活着的 bug ——
  这句话写在用例里,免得下一个人照这段去找一个复现不出来的症状。
- 🟠 `install_plugins` 里那道 `on_boot` 跳过在正常开机路上已经打不着了
  (真正做决定的是 `_plugin_bodies`)—— 留着它给直接调用方,
  但**日志措辞和那一处分开**:两句一模一样的话会让人查错地方。
- 🟠 **REFERENCE 那道默认值闸从只查 `bool` 扩到 `int` / `str`** ——
  装上当场逮出 `director.grace_hours`:表里写 12,而 `_DEFAULTS` 是 24。
- 🟠 `chat_tick` → `chat_seq` **是改名不是加法**:Known 里按消费方写清
  (老名字没了),REFERENCE 那一段跟上。
- 🟡 CHANGELOG 3.11.3 的 `Known` 里 `location_name` 那条重复了一遍,去掉一条。
- 🔴 **3.11.3 那条 `agent_name` 的修复记录说大了,改口** —— 三个发射点本来就
  都填了,而 `agent_display_name` 对非 `player:` 的 id 永不返回空:那条用例
  贴到 `169009f` 上照样绿。改的是结构(一处给 + 永不为空),**不是那个线上
  症状的修法,而根还没找到**。一句说大了的修复记录,比没有那条记录更坏。

### 🔴 被拒的操作也是玩家做过的一次操作 —— 要开口,但不加时刻(老板 09-03 裁决)

真站第五轮量到「连着三屏同一句」。复现出来的根是:玩家被一个长动词占着,
**再点同一个动词一律被拒** → 一条事件都不发 → 时刻钥匙一格没动 → `cached`,
而抬头报的是上一屏那个。**引擎这一处是对的**(世界里确实什么都没发生),
而产品那句「每操作一次就有新剧情」在这条路上不成立。

裁决把两件事分开了,而这条分界现在写在代码里:

| 问的是 | 读哪张表 | 谁读 |
|---|---|---|
| 他**真动了手**没有 | `host.ACTED_GRAINS` | 编剧那道守卫 |
| 这一屏**该不该重写** | `host.SCREEN_GRAINS`(= 上面那张 + `refused_seq`) | 开屏那道闸 |

· 抬头**照旧 `acted`**,`host.moments` 那张表一个字不动(「只在六个时刻开口」
  是结构性纪律,不为一次产品需求破例)。
· 那一屏 `scene.source == "template"` —— **不是 `cached`,也不调模型**。
· **编剧一个字不写**:一拍关于没发生的事的剧情,比不写更坏。
· 说的是引擎**已经给出的那句拒绝人话**(「你已经在做这件事了 —— 还要 1 小时」),
  **不另写一份措辞** —— 同一件事在工具回执和这一屏上说两种话,比其中任何一句错更难查。
· 开场白按第几次**轮着说**(不掷骰子:同一份日志重放两遍要得到同一屏)——
  老板那句底线是**屏不许逐字重复**,而连点三下三屏一字不差,和"屏根本没动"
  在玩家眼里是同一件事。

⚠️ **白按不许顶掉真做成的那件事**:一次里既走成了一步、又点废一个动词时,
他做成了事,那一屏照常写。这一格是承重的,有用例钉着。
⚠️ 水位是**内存态**(和 `_player_chat_tick` 同一条),**不进事件日志** ——
世界里什么都没发生,而一条"他试了但没成"的事件会被折进投影、被编剧当成
"他做了什么"读走。
契约加两格:`contract.director.screen_grains` / `refused_scene_source`。
...
## [3.11.3] —— 三条 🔴 都在真部署最常见的路上,而三条都零用例 (2026-09-03)

验收 A 对 3.11.2 判 FAIL,B+C 补了一批。**共同的形状:一句话只做进了半层,
而另外半层照旧,两处各说各的 —— 屏幕上什么都不少。**

### 🔴 ①「库里那份说了算」只做进了记录那一层

3.11.2 把这句话做进了 `install_plugins` 的**那一张局部名单**,它挡住的只有
「别拿旧声明覆盖库里那一行」。而**运行时装的是哪一份**由 `_plugin_bodies` 决定,
那一处照旧「文件里那份赢」:

    库里记着 2.2.0 · `scheduler.plugins` 跑的是 2.1.0 ·
    2.2.0 那几条规律一条都不在跑 · `projected_facts` 空着 ·
    而日志一本正经地说「库里那份说了算」

**一句写在日志里、却没做进运行时的话,比没有那句话更坏**:它让人以为已经修好了。
上一条用例只断言了**库里那一行**,所以它一直是绿的 —— 新用例断言**运行时**
(`scheduler.plugins` / `plugin_rule_ids` / `projected_facts`),并试过牙。
那句日志同时从 `info` 提到 `warning`(运维台按 warning 抓),而且改说在
**做决定的那一处**。

### 🔴 ② 聊天那格水位是**世界时钟**,而龙族一个 tick 是 4.2 真实分钟

玩家在一个 tick 里聊十轮,`last_contact_tick` 一格没动 —— **十轮里只有一轮换屏**,
而那正是"真站第四轮"报上来的症状本身。
🔴 **而 3.11.2 那两条用例是手喂 `clock + 1` 才绿的** —— 真门传的是
`scheduler.clock`。**那是把测试改绿,不是把东西修好。**
水位改成**一轮一格的单调计数**(`contact_seq`,和那个 tick 同住一行,没有新键),
钥匙那一格随之从 `chat_tick` 改名 `chat_seq`(它不再是 tick,名字就不该是)。
用例走真门、**一个 tick 都不推**。

### 🔴 ③ 在路上那一格读错了字段名

`self.players[pid]` 里没有 `transit` 这一格(它和在场同住一个 hash,叫
`__transit__`)—— 于是 `location_name` 恒为「路上」、目的地一直是丢的,而
**同一屏的场景那句话说得出「去建筑工作室的路上」**(它读 `state()` 那份)。
改读 `_player_transit`(`player_in_transit` 用的同一个来源)。
🔴 那条闸只比了两扇门相等,**两边一起退化时它照样绿** —— 加了一颗牙:名字里要有目的地。

### 🟠 ④ 拆三合一时拆出了一个新的三合一

3.11.2 把 `source` 从三合一拆成四种,可拆完之后 `else` 那一支写着 `refused` ——
而算术选了 `breathe` / 没人可派 / 额度用完 / 撞上锚点 **四种都没问过模型**。
真站 12 拍里 10 条 `refused`,而假客户端只被调过 6 次:**读数自己就对不上**。
判据换成一句能证伪的话 —— **这一拍到底调没调过客户端**;闭集加
`capped` / `anchor` / `no_cast` / `arith`,`doctor` 分开数,
并补一条「`refused` 条数 ≤ 客户端真被调次数」的对账用例(试过牙:退回旧版当场 12 > 6)。
`doctor` 那句「N 拍被同意门或上限挡过」里**"或上限"那半原先永远是 0**(只数
`refused_by`,而顶上限根本走不到同意门)—— 一并数上。

### 🟡 ⑤ 故事那一屏印着两句打架的话,外加一个浮点

    张力 0.00(松弛)· 这条线到节骨眼了 · 编剧一共写过 3 拍
    还没有开着的线。

三宗罪:`0.00` 是给机器看的(而这一屏正是「两样都别自己译」那条规矩的作者)·
「到节骨眼了」和「松弛」**互相打脸**(相位不倒退,张力会衰减)·
紧接着又说「还没有开着的线」——一条不存在的线到了节骨眼。
新增 `director.story_text(tension, phase)` **一处生成**那一句合成的话
(松下来的高潮说「到过『到节骨眼了』,这会儿松弛」),契约 `text_keys` / `story_keys`
各加一格,屏上只印它。

### 🟡 ⑥ 一句把话说满的 docstring

`acted_since` 写着「他自己动过手没有」,而它答的是**日志和水位上看得见的那几格
...
⚠️ **2026-09-06 更正(验收 A ①):上面那句「修了」说大了。**
三个发射点**本来就都**填了 `agent_display_name(...)`,而那个函数对一个非
`player:` 开头的 id **永不返回空**(查不到就退回 id)。所以:

· 这一版改的是**结构**(三处各填各的 → 一处给,加一条"永不为空"的保证),
  它是对的,但它**不是那个线上症状的修法**;
· 那条用例贴到 `169009f` 上**照样绿** —— 它没有牙,而我当时没试;
· **屏上那句「一位还没报上名字的人」的病根不在这三个发射点**,还没找到 ——
  下一步要问的是站点那一侧:它读的是不是这条 `agent_hail`,
  还是 `/state.agents` 取不到人时自己兜的那句。

**一句说大了的修复记录,比没有那条记录更坏**:它让下一个人不再去找真正的根。
...
- REFERENCE 配置表里 `director.enabled` 仍写着 `false`(3.11.2 已翻成 `true`)
  → 改真,**并加一道闸**:表上那一列的默认值要和 `_DEFAULTS` 逐个对得上。
  ⚠️ 上一道闸只查这个键**在不在**表里,查不出**那一格写的是什么** ——
  一张齐全而说谎的表,和一张缺行的表是两种病。
- `contract.director` 补 `acted_grains` / `sources` / `source_labels`,
  `move_event_types` 里那个 `entity_engage` 进 `Known` ——
  **下游探不到的能力等于没交付**:站点为「聊完一轮屏不动」自己加一条每轮事件,
  正是那条硬不变量挡的东西。
- `player story` 那一屏不再印浮点(判据是**去问屏幕**,不是数源码)。

### Known

- 🔴 **「一次被拒的操作也是玩家做过的一次操作」还没有裁决。** 真站第五轮量到
  「连着三屏同一句」,复现出来了:玩家被一个长动词占着,再点同一个动词一律被拒
  → 一条事件都不发 → 钥匙一格没动 → `cached`,而抬头报的是上一屏那个。
  **引擎这一处是对的**(世界里确实什么都没发生),而产品那一句
  「每操作一次就有新剧情」在这条路上不成立。
  要它开口就要动 `contract.host.moments` 那张表(「只在六个时刻开口」是结构性
  纪律),**那是一次契约变更,不该由这一层顺手做掉** —— 先报。
  当前行为由 `test_在忙的时候连点同一个长动词…` 钉住,不会悄悄变。
- 契约是**加法**;`storage` 段一个字没动;作者层 schema 未动。

## [3.11.2] —— 舰队每次开机都带 `--world-file`(第二次,换了一段) (2026-09-03)

### 🔴 破坏性:开机路上的「不降级」闸拿掉了(而 `pack install` 那条留着)

**线上事故。** 龙族的创世文件声明插件 `kasaier` **2.1.0**,而库里已经是 **2.2.0**
(第 1 周包升上去的)。**舰队每次开机都带 `--world-file`** —— 于是插件那道
「不降级」闸在开机路上抛 `WorldSeedError`,**容器 `Exited(1)`**;
而 **3.9.0 / 3.10.2 / 3.11.1 三版都有这道闸,回滚救不了**。

🔴 **这和 3.10.1 那次是逐字同一种病,只是换了一段** —— 那次是**拍**
(「文件里有 beat 而世界已经有 beat」),这次是**插件版本**。病根同样是:
**拿「一次编辑」的语义去 judge「同一份文件又开了一次机」。**

⚠️ **一个修过一次的形状,在另一段上又活了一次** —— 3.10.1 那一轮只修了 `beat`,
没有回头问一句「还有哪几段也会被同一句话咬到」。**修一个形状时,要问的是
「这个形状还长在哪儿」,不是「这一处修好没有」。**

分界照第 17 条那句「库里那份说了算」:

| 路 | 库里那份比文件新 | 为什么 |
|---|---|---|
| **开机**(`--world-file`) | **跳过、说一句、rc 0** | 舰队每天都在做这件事,那不是"一次编辑" |
| **`pack install`** | **照旧当场拒** | 人按下的一次动作,"拿旧声明去盖新数据"仍然是一次真的降级 |

**对创作台的意思**:创世文件里的插件版本号**不必跟着升** —— 发包升上去之后,
那份创世文件可以原样躺着。回执 `docs/FOR-STUDIO.md` §3.67(d)。

⚠️ **顺带销掉一条运维侧的权宜**:事故当天的拆雷办法是拿一份**运行时导出**
(作者层零段)覆盖 `world.cyberworld` —— 它靠**把声明整个拿掉**拆雷,
而代价是**那份文件从此读不出作者当初声明了什么**(创世态被从磁盘上抹了,
而事后补不回来)。**3.11.2 起舰队可以保留原创世文件**;那一步只在老引擎
(3.11.1 及更早)上做,而且它是权宜不是做法。

判据是**舰队那三步**:创世 → 装升级插件的包 → **同一份创世文件再开机 rc 0**。
少了第三步那个洞照样测不出来;试过牙(退回"开机也拒",当场红)。

### 🔴 一份交付了两版、而文档一个字没动的回执

3.11.1 交了**四格契约加法**(`target_curve_keys` / `text_keys` /
`thread_text_keys` / `story_keys`)与**两条新 CLI 出口**(`guidance show` /
`player story`),而 `docs/FOR-STUDIO.md` **一个字没动**(验收 B 逮的)。
**库里有而对方看不见,等于没有** —— 这份文档存在的全部理由,而它这次栽在
自己那条规矩上。补 **§3.67**;§3.66(d) 那张过期的键表同轮改掉。

### Fixed(验收 B / C 对 3.11.1)

- 🔴 **`player story --player 不存在` 印一份看起来完全正常的空故事** ——
  `known` 就是为这件事加的,而**那一屏自己没读它**。
- 🔴 **`guidance show` 印 `{'setup': 1, 'escalation': 3, 'climax': 5}`** ——
  花括号引号冒号全是给机器看的,`setup` 还是枚举名;`STAKE_LABELS` 导进来
  没用上。改读 `PHASE_LABELS`,并给**这两条只读出口**装了一道
  「屏上不许有裸英文枚举 / Python 字面量 / 裸星号」的闸(**别数源码,去问屏幕**)。
- `contract.director` 报 `move_labels` / `phase_labels` / `stake_labels`
  —— **分档表只有一份**,各译一遍的话同一个世界会在引擎屏 / 创作台 / 站点上
  说三种话。
- `pack install` 的 `guidance_warnings` 印两遍(装包那侧 warning + 回执各一份)。
- REFERENCE 补 `guidance show`、`target_curve_keys`,以及**`release` 不许写进
  `target_curve`** 那句。
- 🔴 **§3.65 小节字母乱序,这是第三次被报同一件事** —— 前两次都是"就地调一下"。
  **一件被报三次的事,该有一道闸**:`test_doc_links.py` 新增一条查同节内字母序,
  而它当场又逮出**第四处**(§3.50 的 (b)(a)(c),一直乱着)。

### Fixed(验收 A 对 3.11.1)

- 🔴 **`recent_log` 按人过滤那一格零覆盖** —— 改回 3.11.0 的写法,2371 条**全绿**。
  补用例并试过牙。⚠️ 用例里那一步「p2 连写十四拍把 p1 那条挤出最近 10 条」是
  承重的:**取 10 条再筛**的实现会让这一格在多人世界里永远空着。
- 🟠 **真站 C 派的两条,我上一版没做也没说** —— **不许沉默**,这一版做了:
  · `agent_hail.payload.player_name` 顶着一串 UUID 上屏(根在 `_relation_name`
    兜底 `return pid`)→ **没名字就不带这一格**,让读的一方自己兜底成「你」/「访客」。
    ⚠️ 这和 `entity_display_name` 那条「查不到就退回 id」**有意不同**:
    那儿的 id 是作者写的、人读得懂,而这儿的是机器发的。
  · 首聊建议句「问问 TA 刚才说的那件事」由 `beat_note` 把门,而 `beat_note` 是
    **旁白**不是她说过的话 → 换成由**她真开口说过的那句**(`agent_hail.line`)把门。
    `mock_opening` 那一格早写着这条分界,而建议句那一层没跟上。
- 🟡 **三种情形被记成同一种**:有意 `breathe`(张力顶到安全阀)· 模型答出闭集之外 ·
  真的没配 key —— 都写 `source:"mock"` + `why:"没配 key:模板句"`,而 `doctor`
  照它数。**一个把三种原因合成一句的读数,指不出任何一种修法**(顶安全阀是世界
  该这样、答出闭集之外是提示词要改、没配 key 是去配一把 key)。
  拆成 `llm|mock|ceiling|refused`,`doctor` 分开数。
- ⚪ `guidance show` 那句「不许去的地方」**比闸大** —— 它今天挡的是「站在那儿的人
  不进候选」,不是「谁都不许去」;**一句比闸大的措辞,会让作者以为自己挡住了一件
  其实没挡住的事**。措辞改真。
- ⚪ `test_时钟的内存读法_零IO` **没量 IO** —— 「两个数相等」对一个每次都打 Redis
  的实现也成立。改成真拦 `execute_command` 数一次,并拿真读法当对照。

### 🔴 破坏性(第二条):`director.enabled` 默认从 `false` 翻成 `true`

三个世界升到 3.11.1 之后**编剧一拍都没写** —— 因为它默认关着,而没人去开。
🔴 **老板的主命题在线上根本不存在,而每一块屏幕都显示正常。**

这个仓库的规矩是「引擎默认值全关」,这一条是**有意的例外,不是忘了关**:
编剧不是一个可选特性,它**就是这一版的产品命题**。而且它**没配 key 也照跑**
(整条 mock 路是活的、拿模板句写),所以"默认开"不会让任何世界因为缺 key 变坏。

**对宿主的意思**:升到 3.11.2 的世界**开机就开始写拍**。不要它的世界写一行
`director.enabled=false`(3.11.1 及更早的行为)。

### Fixed(真站第三轮)

- 🔴 **玩家聊了十轮而整屏纹丝不动** —— `conversation` 事件**只在会话关闭那一刻
  发一条**(「整场会话只在关闭时发一个事件」是这个仓库的硬不变量),而站点把
  会话**一直开着**:于是 `move_seq` 一格不动,`host_turn` 一路 `cached`。
  ⚠️ **不许为它加一条「每轮一个事件」** —— 那正是那条不变量挡的东西。
  时刻键新增第五格 `chat_tick`,读的是转录那侧本来就在写的水位
  (`contact_store.last_contact_tick`),**没有新事件、没有新键**。
- ⚪ 在路上而目的地未知时,那一屏说「你还没落个脚」—— 一句**不对**的话:
  他正在走。`mock_scene(in_transit=True)` 改说「你在路上。到了地方再说。」

### Fixed(真站第四轮:聊完一轮屏开了,而编剧一个字没写)

- 🔴 **同一个形状,一版之内第二次。** 「他动过手没有」这个事实有**两个读者** ——
  开屏那道闸(`_host_trigger` 的 `acted`)和编剧那道守卫(「没有『刚』就不写」)。
  3.11.1 验收 A ① 那次是守卫拿**时刻名**当判据(走一步时 `arrive` 赢了抬头,
  于是走路那一半编剧不写);这一次是我**同一轮**把 `chat_tick` 加进了钥匙、
  **只加在闸那一侧** —— 聊完一轮屏重写了、抬头是 `acted`,而编剧照旧一个字不写。
  🔴 **我修 A ① 时只把守卫换成了 `move_seq`,没有回头问一句「这个事实一共有
  几个来源、以后加一个来源要改几处」。** 现在它是一张表加一个函数
  (`host.ACTED_GRAINS` / `host.acted_since`),闸和守卫都调它,**再加一格没有
  第二处要记得改**;闸上另配一张逐格驱动表,加一格而忘了写驱动当场红。
- 🔴 **一句让运维侧查错了一整轮方向的契约,补进文档。** 这一趟闭嘴时 `trigger`
  报的是**上一屏那个抬头**,于是屏上出现 `trigger:"return"` + `source:"cached"`
  —— 它的意思是「什么都没发生」,却被读成「回来那一屏把编剧短路了」。
  行为**不改**(返回的确实还是上一屏,而 `ask_ready` 要照它说真话),
  **也不加第二个布尔**(同一个问题两个读数,下一次就是它们说两句话):
  写死「问『变了没有』只有 `scene.source`」到 REFERENCE 与 FOR-STUDIO §3.67(f),
  并补一条用例把这个回声钉住。

### Fixed(真站第四轮:编剧真活了,而老板那三句判据一句没兑现)

- 🔴 **① 六件事里三件 `scene.source=cached`,同一句台词连出三屏。** 两条根:
  · **长动词点下去只发 `entity_engage`** —— 那条 `entity_interaction` 要等一小时后
    收尾才发,于是「拉票」点完 `move_seq` 一格没动。⚠️ 我 3a 那条二十连击的用例
    点的**全是短动词**,所以它一直绿着 —— **「试牙也要试对地方」,这是第三次**。
  · **聊完那一屏和上一屏一字不差** —— 屏是重开了(`chat_tick`),可回顾折自日志、
    而聊天不进日志。**一屏没变化和一屏没开口,在玩家眼里是同一件事。**
    改成读开屏用的**同一个水位**取「刚跟谁说过话」,不另攒一份。
- 🔴 **② 两个玩家做了完全不同的六件事,而故事页线数、相位、张力、对手全一样。**
  根**不在模型**:编剧手上只有「上一屏之后」那一条,两个人喂进去的输入一模一样。
  **故事的分岔在输入里。** 两条:
  · 编剧输入新增「他这一路走过来做过的事」(最近 6 步:走到哪、跟谁说过话、
    做过什么动词),窗口封顶 400 条原始事件 —— **它挂在每一屏上,不封顶就是
    一次全日志扫描**。
  · `select_cast` 加第五道**软**闸:连着两拍不再派同一张脸(筛空了整份还回去 ——
    「不许沉默」是硬纪律)。⚠️ 例外只在**线该收了**时成立:上一版拿「有开着的线」
    当例外,而那个条件几乎永远成立 —— **一个永远成立的例外不是例外,是把闸关了**
    (真站上 4 次 approach 全指同一个人,而池子里有 13 个)。
- 🔴 **③ 那一屏复述的是入场那一拍,不是他刚做的事。** 根在一行:截断留的是
  `lines[:RECAP_LIMIT]` —— **最老的六条**,而入场那一拍一口气写四条就占满了名额。
  **这一段的名字就叫「刚发生了什么」,而它留的是最不刚的那几条。**
- 🟡 **④ 三拍就 `climax` 而张力 0.1** ——「到节骨眼了」和「松弛」并排印在同一屏上,
  两句都是引擎说的。相位改成**跟着张力走**(没到这一相的目标就不翻篇,而这一拍
  照写、照加张力),另配一道闸量「写下那一拍的那一刻」两句话不打架。
- 🟡 **⑤ 在路上时 `place_name` 空。** `location` 空是对的(他不在任何地点里),
  而 `location_name` 是**给人看的**那一格 —— 空着的话同一屏上「你正走在去建筑
  工作室的路上」说得出,而地名那一格说不出。补在 `player_options`(不是主持人
  那一屏):**两扇门对同一时刻必须说同一句话**,补在上面那扇门里就是让它们分岔。

### Known

- 契约是**加法**(三张人话表);`storage` 段一个字没动;作者层 schema 未动。
- **`director.enabled` 的默认值变了**(见上)—— 这一条不是加法,升级前先读。
- ⚠️ **`contract.director.move_event_types` 多了 `entity_engage`**(长动词起头
  那一条):一个消费方按这张表判「哪几种事件算他动了一次手」的,要重新读一次。
- 🔴 **`host_scene.payload` 那一格 `chat_tick` 改名 `chat_seq`,这是改名不是加法。**
  它不再是一个 tick(世界时钟),是**这个人一共说过几轮话**的单调计数 ——
  名字不改的话,下一个读它的人会拿它去和 `world_time.tick` 比大小,
  而那个比较**恒为真**且不报错。
  消费方要跟的只有一条:**读 `chat_seq`,别读 `chat_tick`**(老字段没了)。
  ⚠️ 升级那一刻老 `host_scene` 行上没有这一格(读作 0),于是第一次聊天会多开
  一次口 —— 那是对的(他确实说过话而屏幕没说),而且只多这一次。
- ⚠️ **`player_options.location_name` 在「他在路上」那一帧从空串变成「去 X 的路上」**
  (`location` 照旧空)。拿 `location_name` 是否为空当"在不在路上"判据的消费方要改 ——
  那个判据本来就该读 `blocked == "in_transit"`。
...
### 顺带修完的(验收 A / B / C 对 3.10.2)

- 🔴 **`pack list` 把一份只改人设 + 补记忆的包报成「什么都没装进去」**(C ①):
  `_merge_section_ids` 只拍五个段,缺 `personality` / `memories` / `guidance`。
  ⚠️ **这和 A② 是同一个形状、只是少的格子不同** —— 病根都是**那张表是手抄的**。
  闸补上:**回执里报了落地的段,必须进得了 `sections`**。
- 🟡 舰队每次重启上屏一处裸星号(C ②)—— 那道判据只敲过 `pack` 子命令,
  **一次都没敲 `simulate --world-file`**,而舰队走的正是后者。
  「别数源码里的星号,去问屏幕」这条,**只覆盖我记得去敲的出口时和那道 AST 闸
  是同一个毛病**。用例扩到那条路上。
- 🟡 `host_turn` 的 docstring **三处同时过期**(C ⑤):`trigger` 缺 `return`、
  顶层缺 `ask_ready*`、一项缺 `who` —— 而 `who` 正是 3.9.0 漏到站点三处去的那一格。
  **同一道闸现在也扫 `api.py`**。
- 🟡 §3.65 (c)(d) 对调之后两处引用没跟(C ④);`door_params.chat` 表缺 `opening?`(C ⑦)。
- 🟡 **死链闸放宽到 `docs/` 打头**(C ⑥),顺手扫出**十二条**同族死链
  (CHANGELOG / REFERENCE / FOR-STUDIO 指着父仓库与兄弟仓库的文档)。
  ⚠️ 闸的判据是**可证的**:`docs/…` 只在「同一条路径在父仓库里真的存在」时才报 ——
  否则它会咬到「诉求写进**你们自己**的 `docs/…`」这种讲别人家路径的话,
  而**一道分不出"链接"和"讲别人家路径"的闸,会被人关掉**。
- 🟢 AOF 警告一次开机印两遍(两处 emitter)· `beats.py` 三处裸英文 + list repr ·
  `simulate --world-file` 路径打错甩 Traceback → 中文两行 + rc 2 ·
  建议句提示里写死的「他」→ 用名字(**不猜性别**)。
...
- **一个装过剧情的世界,第二次开机起再也起不来了。** 3.10.0 把「文件里有 `beat`
  而这个世界已经有 `beat`」判成当场拒绝(退出码 2)。**那条拒绝的理由是对的,
  错的是它问的问题**:判据取的是 `beats_store.seed()` 有没有播下去,而 `seed()`
  的语义是「空的时候才播」—— 于是**「同一份文件第二次开机」和「一份带着新剧情
  的包」在它眼里长得一模一样**。

  🔴 **前提是这一句,而 3.10.0 的破坏性说明漏写了它**(验收 B 逮的;这句话是
  §3.61 `edge` 那一节八天前的原话):**舰队每次开机都带 `--world-file`。**
  带上它,3.10.0 那条判据的下场就是显然的 —— 第一次开机装进剧情,第二次开机
  文件里那几拍已经在库里,于是 rc 2。**线上龙族因此起不来**(platform 现场撞上,
  已回滚)。回执 `docs/FOR-STUDIO.md` §3.65(a)。

  判据换成**逐拍比**(`beats.split_against_stored`,纯函数),三堆各有各的反应:

  | | 反应 | 为什么 |
  |---|---|---|
  | 同 id 且内容相同 | 静默跳过,rc 0,至多一句「已经在库里」 | 舰队上的常态,不该说得像出了事 |
  | 同 id 而内容不同 | **说一句,照常开机** | 库里那份说了算(`:beats` 的既有契约),而作者需要知道他的改动没生效 —— 一次静默的「改了没生效」是这一族最贵的错法 |
  | **新增**的拍 | 照旧 rc 2,并指向 `pack install` | 这才是 3.10.0 真正要挡的:一份新剧情走 `--world-file` 混进跑着的世界,零点会是世界第 0 天,那几拍在同一 tick 里全烧掉 |

  离线那两扇门(`validate world` / `world check --edit`)的措辞同一轮跟上 ——
  它们从前说的是「有的话开机是『当场拒绝』」,而那句话现在只对第三堆成立。
  **三扇门说同一句话**这条纪律在措辞上也要成立。

- **装包路上插件声明的种类先并进 `kinds` 再做本体预检**,以及**同一个插件升版时
  本体重编译一次**(第 15 条那两半)。⚠️ 这两件**是 3.10.0 交付的**,上一版只改了
  `docs/REFERENCE.md` 两行 —— **库里有而创作台看不见,等于没有**,所以这一轮补回执:
  `docs/FOR-STUDIO.md` §3.65(b)。晚一步并种类的下场是一个引用插件种类的
...
- 这一版**只改判据,没改契约**:`contract --json` 逐字未动(`storage` 段尤其),
  作者层一个段没加。所以它是一次**补丁版**,消费方不必做任何能力探测上的改动。

## [3.10.0] —— 周更:创作者往一个跑着的世界投一份更新 (2026-09-02)

老板 09-02:「我要的是创作者能控制这个东西,相当于每周都能有更新」。
上一单(玩法层批 1)的龙族是**重建**上线的 —— 这一版之后不再重建。
裁决全文 `docs/FOR-STUDIO.md` §3.62;任务单
`../../docs/任务单/2026-09-02-周更链路-批2a.md` §3.1 是这一波的边界(2a-①)。

**抬版号的理由和 3.8.0 / 3.9.0 那两次逐字相同**:`contract --json` 加了一段
(`packs`)加一格(`beats.since`),而消费方正是拿它做能力探测。
🔴 **这里原先写着「`storage` 段一个字没动(实测与 3.9.0 逐字相同,六格)——
运维台那条 deepEqual 不会红」,而那句话是假的**(2026-09-02 验收 B 逮的,
3.10.1 这一轮改)。它把**两件事**混成了一句:`storage` 的**顶层六格**确实没增减,
而 `volatile_keys` 的**内容**多了一格 `config_rev` —— 底下「破坏性」那一节
自己就写着这件事,两段在同一份文档里互相打脸。

**实测**(2026-09-02,拿线上那支 3.9.0 容器与本地 3.10.1 各敲一次
`contract --json` 比的):3.9.0 的 `volatile_keys` 是 4 条
(`lock` / `players` / `player:{player_id}` / `erasure:{player_id}`),
3.10.0 起是 5 条(多 `config_rev`)。所以 **`.storage` 变了,运维台那条
`deepStrictEqual` 会当场红** —— 那是它该有的样子,以底下那一节为准。

⚠️ 教训是**度量单位**的:「六格」数的是顶层字段数,「一个字没动」说的是逐字节
相同,而一句话同时用两把尺就必然有一把在撒谎。**下次写这句话之前,把两支引擎的
`storage` 段 `diff` 一遍**,别数格子。

`:packs` 不存在、「装了哪几周」折自事件 —— 这半句仍然成立。

⚠️ **破坏性三条**(第三条是验收 B 逮的,上一版这里只写了两条):
- 带 `pack` 记录的包 `engine_min` **必须写 `3.10.0`** —— 老引擎见到不认识的
  作者层 `type` 是**开不了机的硬失败**(和 `beat` 3.7.0 / `plugin`+`edge` 3.8.0
  逐字同一种,响的)。3.10.0 起**引擎自己会查这一格**:写一个更低的数是当场拒
  (三扇门同一句),没写是一句警告(「没说」和「说错」是两件事)。
- 带 `beat` 的包走 `--world-file` 装进一个**已经有剧情**的世界,从 rc 0 变
  **rc 2**。打到的是任何拿 `simulate --world-file` 当"给跑着的世界装东西"用的
  脚本;判包那条路走 `world check`,**不受影响**。
- 🔴 **`trigger` / `trigger.at` 收成闭集,而开机会把库里存量的拍重验一遍** ——
  一个 3.9.0 上跑得好好的世界(那一版这两层一个键都不查,写错一个字母是照收然后
  丢掉)换上 3.10.0 会 `BOOT FAILED`。**已修成"存量拍宽容"**:库里那份只警告
  (`doctor` 数得到),新文件 / 新内容包照旧严格 —— **一次收紧不许把已经发出去的
  世界锁在门外**。升级前想先扫一遍存量,判据是这一条:

      anima-world contract --json | jq -r '.beats.since.at_keys, .beats.since.trigger_keys'
      # 再拿 `zcat <世界包> | grep '"type": "beat"'` 逐条对照那两张键表;
      # 或者最直接的:拿 3.10.0 真开一次机,`stored=True` 那几句 warning 就是清单

- ⚠️ **`storage.volatile_keys` 多一格 `config_rev`**(配置表改过几次,`ConfigStore`
  拿它判断"我手里那份还新不新")—— **进程态,不进 `.cyberworld`**。
  🔴 **运维台那条只比 `.storage` 的 `deepStrictEqual` 会当场红,那是它该有的样子**
  (镜像本该在这种时候喊),同轮认账。

### 加了什么

- **作者层第十五个段 `pack`**(`{id, version, note}`,对象型 —— **一个文件就是
  一个 pack**)· `pack_installed` 事件 · `Projection.packs` / `World.packs()`。
  **没有第二张表**:「装了哪几周」折自事件,和余额折自 `payment` 逐字同一种。
  `pack_installed` **不进** `contract.plugins.subscribable_events`(策展的十条):
  它的当事人是世界**外面**的人,订它的插件做不出任何世界里的事。
- **`World.install_pack(path)` + `anima-world pack install|list`**。它和
  `--world-file` 的分界**不是「装什么」,是「谁在装」** —— 装包的那个进程就是在跑
  的那个进程,于是「别的进程装的东西这个进程看不见」这个问题整个消失。
  新开三段:**剧情拍**(按 id 合并)/ **作者动过的开关** / **世界观**;
  其余段照旧只填缺不覆盖,**走开机那条路上同一批播种函数**。
- **拍的零点跟着来路走**(`trigger.at.since`,闭集 `pack`/`world`,缺省 `pack`):
  创世那批 = 世界第 0 天(逐字如旧)· 一份 pack 装进来的 = 那个包**第一次**落地
  那天 · `for_each: {"node": "player"}` = 他入场那天 · **两者都有 = `max`**。
- **`:config` 跨进程可见**(`:config_rev`,tick 边界 + 只读门重读)。
- **那五段不再静默**:`beat` 当场拒绝,`config` / 在册者 `personality` /
  `world_setting` / 在册者 `memory` 各一句明白话,**三扇门共用一份常量**。
- `trigger` / `trigger.at` 两层的键收成**闭集**(此前一个都不查)。
- 橱窗带一条 `pack` 记录(「做了却开箱看不见等于没做」),封皮两个数 → `3.10.0`。

### 🔴 这一版最该记的四句

1. **零点不是一个可选特性,是「合并剧情」这件事的前提。** `trigger.at` 的语义是
   「不早于」,所以一份写着 `day: 0..6` 的第 2 周包装进一个跑到第 40 天的世界,
   **八拍在同一 tick 全部烧掉**,零报错 —— 而 `beat_fired` 是历史,烧掉了就再也
   回不来。这正是引擎从前拒绝合并节拍的**唯一正确理由**,不是一句托词。
   **`max` 是唯一能同时让两句话成立的写法**,两半都试过牙(退回任一半当场红)。
2. **一句写在日志上的真话,和一盏假绿灯是同一件事。** 2026-09-02 实测:一份内容
   全落在那五段上的第 2 周包,`world check --edit` 答 `loadable: true, errors: []`、
   `simulate --world-file` 退 **0**,而世界一个字都没变。**同一个 `not persisted`
   门后面站着五个人,只有第一个会出声** —— 创作台的诉求 C 量到了前三个。
3. **两份真相里有一份不更新,而它最安静的样子是「另一个进程改了配置」。**
   `ConfigStore` 开机 hydrate 一次进缓存,于是运维改了 tick_rate,正在服务玩家
   的那个进程照旧按老的跑,直到有人重启它。修法照 `:entities_rev`,而调用点是
   **tick 边界与只读门,不是每一次 `get()`** —— 实测 `get` 每 tick 约 6 次,
   一个 60 天快进就是几十万次往返。**「一个 tick 之内配置不变」是有意的语义。**
4. **一份没人验的键表,和一句没人验的话是同一种东西。** REFERENCE §4.8 那句
   「顶层十三段」在 3.9.0 加 `host` 之后就烂了,而没有一处会红 —— 那一行是三个
   下游仓库照着写探测器的地方。补了机器闸,判据是**逐个名字不是条数**。

### 2a-②:人设按 CAS 覆盖 · 记忆只增不改 · 停用 · 主持人第五个时刻

- **人设 compare-and-set**。「同一个 pack 就能覆盖」是错的,而它错得不报错:
  第 1 周的包给她写过一句人设,玩家跟她聊了三十天、人设被 `persona_update` 改过,
  第 2 周包一升级就把那三十天抹了。判据是**「这一格此刻的值还等于我上一版写下去的
  那个值吗」**;不是我写的、或者被世界改过 → 默认拒绝,`--force` 才写(老板 D53 ④)。
  问的是**投影不是黑板**:黑板那份是开机拼出来的快照,而 `persona_update` 是事件。
- **在册的人的记忆只增不改**,按 `(agent_id, summary)` 去重。记忆是演化态,改一条
  既有的等于伪造历史。⚠️ 这条路不调 `_rebuild_memories`,所以刚追加的那几条得**自己
  折一次** —— 照抄开机那条的 `count() > 0` 守卫会让它们留在日志里而进不了库。
- **K7 停用**(`World.disable_pack` / `anima-world pack disable`):拍不再响、
  它带来的新人**退场**(走 `agent_leave` 那条已有的路)、开关**回落到装包前那个值**。
  🔴 **停用不是删除**,而这一条是硬的:玩家的记忆里有这一周发生过的事 ——
  只追加一条 `pack_disabled`,朝后看的一个字不动(`forget_player` 同一个形状)。
  ⚠️ 开关**只回落至今还等于这份包写下去的那个值的那几个**,装完之后被人调过的
  留着并点名(`kept`)—— CAS 在这一层的同一把尺。再装一次 = 重新启用。
- **主持人第五个时刻 `return`**(`host.moments` 四→五,新配置键 `host.away_ticks`)。
  🔴 **零新状态**:他刚才在不在读在场行(带 TTL),有没有新包读 `pack_installed`。
  ⚠️ 两条判据各栽过一次:① 第一版拿「现在的 tick 减他上一屏那个 tick」当离线,
  而一个**一直在玩**的人拿到的多半是 `cached`、`last.tick` 根本不往前走,于是世界过了
  一天他会被告知「你有 1 天没来了」——**「世界走了多久」不是「他离开了多久」**;
  而且「他刚才在不在」必须在 `_touch_player` **之前**问,那一句本身就会把 TTL 续上。
  ② 新包那一条要按 `seq` 比不按 `tick` 比 —— 装包和上一屏可以在同一个 tick 里,
  而那一趟正是最该说「本周更新」的一次。

### 批 1.2:世界先开口,而这一条要开到**对话里**去

老板 2026-09-02 刷新之后真进去玩,原话:**「让我去跟他们说话我不知道说啥,剧情没法
往下走啊,不让他们自己搭话吗」**。

🔴 **「世界永远先开口」这条纪律,3.9.0 只做进了主持人那一屏,没做进对话里** ——
玩家点「跟夏说说话」,拿到的还是一个空白输入框,而跑团桌上 GM 不会把 NPC 推到你
面前然后闭嘴。三件:

- **`World.chat_open(agent_id, player_id, *, hook=…)`** —— 一场对话的第一句由角色说。
  **共用整套提示词**(同一个 `ChatService.respond`),只多两样:一条「这一轮你先
  开口」的系统指令 + 这一刻的由头。静音闸 / 身份 / 在场 / 联系水位**一格都不跳**
  —— 它们答的是「这一场对话成不成立」,和谁先开口无关。没 key 或空回复 → 一句模板。
  主持人那一屏 `talk` 项的 `door.params.opening` 为真时宿主就该走这条。
- **beat op `hail`** —— 让一个角色主动来找玩家搭话。**一条新路都不开**:走的是
  `agent_hail`(站点已经在听)+ 把她真的挪过去。`target` 只写得下 `player`
  (世界文件写在玩家出现之前,一条拍写不出他的 id;而「角色去找角色」是行为树那条
  路的事)。频率沿用 `claim_hail`,和 `reach_out` 共用一个水位 —— 像个人不像推送;
  **没叫成不等于这一拍作废**。
- **`World.chat_suggestions(agent_id, player_id)`** —— 输入框上方 2–3 句他可以说的话。
  **挑什么是纯算术,LLM 只写字**(和主持人挑选项逐字同一条);没 key 时种子直接
  就是建议,可复现;**永远不空**。

⚠️ **拿真 CLI 敲一遍逮到一条测试没写的**:`line`(作者写在 `hail` 上的**她的台词**)
和 `narrate`(拍上的**旁白**)被当成了同一种东西,于是「手机震了一下,是个没存过的
号码。」被塞进引号里当她的台词念出来。**一句念不通的话和一句错的一样贵。**

### 🔴 三视角验收退回一轮,而它逮到的十四条里有六条测试全绿

**全量 2235/19/0 全绿、六条验收标准逐条真敲全过,而验收 A/C 各真敲出一族测试没走到的路。**
这一段留着,因为那六条的形状值得记:

1. **一份只带一拍的包会把引擎内置的地图和名册灌进跑着的世界。**
   `_seed_world_defs` 的「没写 = 回落内置那份」对**创世**是对的(一个没写
   `locations` 的世界总得站得住脚),对 `install_pack` 是错的 —— 地图上凭空多出
   `cafe`/`home`/`workshop`,还跟着三条 `location_join` 进日志,而**事件是只追加的,
   撤不回来**;不带 `agents` 更阴:`:bt_actions` 多出 `chat_with_夏/柔/遥`,
   指着三个这个世界里不存在的人。全程零报错,`world check --edit` 也不说。
   **同一个默认值,在两条路上一条是对的、一条是错的** —— 而那两条路共用一个函数。
2. **同一个 pack 升级把上一版那几拍的零点打回世界第 0 天。** `sections` 是整片
   替换,于是升级那一刻上一版的拍从 `pack_days_from` 上消失,零点读作 0,
   下一 tick 全烧。⚠️ **那条升级用例装的两份都不带 `beats`,所以它看不见** ——
   「一条试牙试不到自己所修那件事的用例,和没有这条用例一样」的第五次。
3. **两份真相**:同一份包回执 `agents: []`(对),而 `pack list` 报
   `sections.agents: ["夏"]`(抄的是文件)。现在 `sections` = 落地,`declared` = 文件。
4. **在册的人的 `personality` / `memory` 一个字不说、rc 0** —— 而**离线门反而说了
   两句漂亮的**。逐字违反同一轮自己写下的「五段当场说」。
5. **「装不进当场抛且一个字节不写」在插件路上不成立**:`except PluginError` 排在
   锁里六次写之后。**半装进去一份包比装不进去坏得多**。
6. **一次收紧把已经发出去的世界锁在门外**(破坏性第三条,见上)。

⚠️ 还有一条是**屏幕**:`since: "world"` + `day 0..6` 装进第 40 天,七拍同 tick 全烧
而 rc 0,屏幕上还印着反话「`day: 0` 的那一拍下一 tick 就响」。装的时候手上有
拍表 + `since` + 今天,**算得出来就不许让它安静地发生** —— 现在默认拒绝、`--force` 才装。

### ⚠️ 过程里自己栽的三次(留着当标本)

- 新常量 `TRIGGER_KEYS` 和 `plugins.TRIGGER_KEYS` **撞了名字**,而 `run_contract`
  把两边 import 进同一个作用域 —— 契约里那一格印出来的是插件那五个键。
  **撞了不报错**,它只是安安静静地答另一个问题。
- 离线那一段第一版只塞进了 `validate world`:锚点字符串两处都有,而
  `replace(…, 1)` 只换了第一处。测试逮住了,因为它断的是「**两扇**门都说」。
- 试牙时拿 `git checkout <文件>` 去回滚,把同一份文件上**还没提交的另外三处改动
  一起抹了**。试牙要用备份文件,别在有未提交改动的树上用 `git checkout`。
...
### 作者层第十四个段 `edge`:一条边种得下了(2026-08-31,收件箱 D44)

**补的是一处不对称。** 内核自己的关系图有作者层入口(`relation` 段),而插件的边
没有 —— 于是「门派的初始成员」「谁一开始就收着哪盒磁带」这种**创世态**写不出来,
只能靠运行期动词 `link` 长出来。这个缺口是 2026-08-27 那一轮**自己量出来并记下的**
(`contract --json` 的 `plugins.deferred_author_sections`,连理由一起报),
今天照 D44 兑现。

**不开的代价不是"少一个功能",是把作者往写一件假东西上推**:配一个只为创世存在的
动词让人手点一遍,或者写一个假触发器绕它。

- **`{"kind": "author", "type": "edge", "body": {"type", "from", "to"}}`** —— 三个键,
  一个不多。`type` 是 `<插件>.<边名>` 全名,两端是**节点 id**(形状读
  `plugins.edge_node_id_forms`)。
- **种边走 `Scheduler.apply_edge_effect`,不自己 `store.link`。** 那个函数里住着三件
  这条路同样要的东西:`exclusive` / `exclusive_to` 的约束、声明过的事实**带命名空间**
  落地、`symmetric` 建两条而不是一条 —— **各漏一件都不报错**。
- 🔴 **`facts` 有意不收,写了当场被拒。** 运行期那条路上「声明的默认值带命名空间、
  效果里手写的 `facts` 不带」**今天就不一致**,收作者层这一格等于把一个已经在打架的
  语义再复制一份。那处不一致本身是一条独立的账,记在 FOR-STUDIO §3.60。
- 🔴 **出厂插件的边一律拒**(`invitation.*`):它们是内核**投影的物化视图**,每次开机
  照事件日志重建 —— 手写一行要么下一秒被抹掉,要么就是**伪造演化态**。
- **只填缺不覆盖,粒度是「每一种边」不是每一条**,而这个粒度是**量出来的**:舰队上的
  世界容器每次开机都带着 `--world-file`(在真进程上看过),逐条补会让每一次重启都把
  运行期断掉的边接回来 —— 退了师门的人容器一重启又是弟子,零报错。整种跳过时
  **点名说出来**(照节拍那一段的安排:一份不合并的东西,宁可大声不做,不要安静地做一半)。
  ⚠️ 剩下那个角落如实记着:一种边**每一行都被断掉**之后再开机,它会整批重新种下。
- **离线与开机同一份判断,而分界由数据自己给**:这份文件里有那个 `plugin` 记录 →
  逐条查到底;没有(一次编辑最常见的形状就是只带几条边)→ **明说这一格答不了**,
  不猜一个答案(开机拿的是「出厂 + 库里 + 文件」合并后的名单,它答得出)。
  ⚠️ 加一种开机失败就必须同一轮补进离线那两扇门 —— `world_edge_errors` 进
  `AUTHORED_LAYER_CHECKS`,外加 `tests/test_validate_matches_boot.py` **八之三**
  那份逐条枚举(八种拒绝),因为**在表上不等于覆盖了开机会拒的每一种**。
- **`STRICT_LEVELS` 多第十二层 `authored_edge_keys`**,而它住在**作者层的 `edge`
  记录**上、不在 `plugin` 记录里 —— 按记录类型给那张表设边界,正是"一层一层收"那个
  bug 的形状(新开的那一层恰好在边界外,于是又一次不在名单上)。
- **`uncreatable_edges` 改口**:作者层种下的那几种也算"造得出"。不改的话,一份写着
  初始成员的世界会收到一句**假警报** —— 上一轮那条用例的 docstring 就写着这件事。
- **契约 72 → 76 格,纯增量**:`edge_author_type` / `edge_author_section` /
  `authored_edge_keys` / `edge_author_gloss`;`deferred_author_sections` 里那条 `edge`
  撤掉,**而那一格留着并且是空的** —— 空对象 ≠ 整格缺席,删了消费方就分不出
  "这一版不欠段"和"老引擎答不出这个问题"。
- **`engine_min` 仍是 `3.8.0`,没抬到 3.9.0** —— 而**理由重写过一次,因为第一版是假话**。
  🔴 **勘误(同日,验收 B 逮的;原话也写进了 commit `5b8a333` 的正文,那儿改不了,
  以这里为准)**:上一版写的是「`3.8.0` 从没打过 tag、没上过 PyPI、**线上也还没有一个
  3.8.0 镜像**」—— **第三条腿是假的**。当天 `docker images` / `docker ps` 现敲:
  `anima-world:3.8.0` **在**(`bb58e30870e3`,08-26 从 `core@13d5078` 建),
  **两个线上世界容器此刻正跑着它**,而那支引擎 `contract --json` 的 `plugins`
  **只有 22 格**、没有 `edge_author_type`、`deferred_author_sections` 整格缺席。
  **「同一个版本号下两支能力不同的引擎」这笔账今天就已经收到人了**,而且不是这一轮
  收的 —— 62 → 72 格那 19 个 commit 就已经在这个名字下换过一支引擎。
  **一条推迟的理由应该在推迟的那一刻就量**,这句话这个仓库写过一次,而我这次又没量。
  **判决仍是不抬版,理由换成站得住的那个**:抬版要动 platform 的镜像 tag 与回滚目标、
  tool 的 `cores/<版本>`、以及波 2 任务单里写死的「重建 `anima-world:3.8.0`」——
  **是一次跨仓契约变更**,按「契约变更先报不先做」不由写者单方面裁,已浮给调度台。
  **在它被裁之前,危险由一条判据挡着**:*重建镜像之前,任何带 `edge` 的包不得发舰队*;
  判据是 `plugins.edge_author_type` 在不在,**不是版本号**(两支引擎同名,版本号在这里
  一个字都答不了)。

⚠️ **「没写 `edge` 段的老世界文件逐字节不变」这条,量法换过一次。** 第一次拿
`export` 在改动前后各导一份来 diff —— 而 **HEAD 对它自己就不相同**(记录里带
`updated_at` 墙钟、叙事跑在线程池上、事件条数每趟都在动)。**那把尺子量到的是那两样,
不是这次改动。** 判据改成两件确定的事,都真跑过:内置橱窗那几条作者记录读一趟再写
出去逐字节相同、一份没写边的种子往返之后**不会凭空多一个 `edges` 段**(空列表和缺席
是两个意思);外加老世界开机后的 Redis 键集改动前后逐字相同。

### 第三波那次修法自己开出来的两个洞(2026-08-28,A 视角 FAIL)

两条都是**回归**,而它们是同一种错的两面:**一份判断有两个副本 / 一份名单和它要判的
那份数据来自两次不同的合并。**

- 🔴 **动词读自己挂在 `world` 上的事实被判成"读了别人的"**:A1 只在**读的那道闸**里
  剥了 `world_`,而同一轮**我自己新加的**动词名字闸没剥 —— 同一句 `world_wet.潮位`
  写在触发器里过得去、写在动词里进死胡同(「写进 reads」→「没有装 `world_wet`
  这个插件」)。**同一条死胡同我在两处各修过一次,而第二处是我自己开的。**
- 🔴 **插件世界之后任何带 `kinds` 的增量编辑都开不了机**,报的是作者没碰过的那一行、
  还断言「装着的是(一个都没有)」,而那个插件就在库里。病根:喂给本体的是**库合并
  之后的全集**,名单却只取**文件**那一份 —— 正是 `_seed_ontology` 自己 docstring
  反对的那件事(喂全集、判局部)。开机那一趟的名单现在 = 文件 + 库里 + 出厂,
  和真正要装的那份插件名单**同一次合并**算出来的。
  ⚠️ 这条直接打在创作台的增量编辑流程上,而「让作者把 plugins 段抄一遍」不算修。

顺带更正一句我写错的因果:「只认库里那份会第一天红、第二天绿」是错的 ——
第一天开不了机时插件根本没进库,**红/红**。回执 `docs/FOR-STUDIO.md` §3.59。

### 第三波:一次放行开出来的三个洞,和三句念不通的话(2026-08-28,三视角验收量全的)

§3.52 那次「本体层放行带命名空间的名字」**只补了键那一半,而放行是有半径的**。

- **A1** 插件**自己**挂在 `world` 上的事实,在它自己的触发器里三种写法全不通:
  `world_wet.潮位` 开不了机、而拒绝语说「读了别的插件的 …,要读就写进 reads」——
  照着改,下一句变成「没有装 `world_wet` 这个插件」。**一条把作者引进死胡同的
  拒绝语,比没有拒绝语更贵。** 修:读的那道闸也剥 `world_` 前缀(和剥 `me_` 同一处);
  另两种写法(装载期全绿、运行期每来一条事件炸一次)当场拦,**并把该写的那串给他**。
- **A2 / A3** 那次放行**只认形状**,于是同时放开了两件:一个不存在的插件
  (`me_qi.灵力`,开机绿、运行期每一次 `ok: False`)、以及**一个没有插件的世界**里
  的作者层写法。判据换成「这个命名空间在这个世界里有人声明过」;
  ⚠️ 名单**两个来源**(这份文件 + 库里此刻装着的)—— 本体是每次开机重新解析的。
  ⚠️ **这里第一版写的「只认库里那份会第一天红、第二天绿」是错的**(同日 A 视角实测):
  第一天开不了机时插件**根本没进库**,第二天照样红 —— **红/红**。
- **B5 裁决:`reads` 在动词表达式这条路上承重。** §10.2 把「读别人的要 `reads`」写成
  **这一层的边界**,而动词这条路从前整个绕过它 —— 一条写在文档里而某条路不守的边界,
  比没有这条边界更坏。🔴 顺带量掉我自己写的配方:`economy.coins` 那个例子**今天根本
  读不到**(出厂插件不是作者记录,`reads` 指不着),gloss 已改成敲过一遍的那份。
- **B4** 拒绝语把 `me_mana.魔力 >= 5` 绞成「你的mana 5」:`ast.walk` 把 `Attribute`
  和它里面的 `Name` **各换了一次**,两处 span 重叠而替换是盲改的。现在念作
  「你的魔力 >= 5」。**一句念不通的拒绝语和一句错的一样贵。**
- **C1** `emit.when` 写成列表报「表达式是空的」—— 那句话在说一个**他没写错的地方**。
  改成说形状(一句 vs 一列)。
- **账目**:`trigger_stats()` 是**七个**数不是六个(三处改)· `emit_key_requires`
  补进 REFERENCE §10.8 那张表 · §3.57 的「tools 块」是错词,种类实例与动词出现在
  **感知块**。

契约**零加格(仍 72)**,只有 `namespaced_write_gloss` 的文字改了。
回执 `docs/FOR-STUDIO.md` §3.58。

### 第二波 ⑧:一个「一个数都没有」的东西,她整条看不见(2026-08-27,创作台验收 C 撞的)

`group:` 种类的实例与角色同处一地,而她的工具块里**整条不出现**,于是组织类插件的
动词谁也调不动 —— 而 `ontology` / `validate` / 工作台**三处都说那个动词存在**,
`World.act` 直调也真的通。**三处说有,而没有一个角色到得了它。**

🔴 **诊断被这一趟更正了:病灶不是 `group:` 那个前缀。** `group` 那个记号在感知
那一路上根本没被过滤 —— 同一个门派,只要身上有一个 `here` 的量,它和它的动词一直
都进得了提示词。真正的那一行是 `perception.perceive` 里的 `if not seen: continue`,
而这一块带的**不只是量**,还有**名字、gloss 和动词**,那三样一个都不依赖量。
于是一个身上没有数的东西(门派、组织、一块牌子)**整条不存在**;它对 `entity:`
同样成立,只是组织天然没有数,所以撞上的是 `group:` 那一面。

**判据从「有没有数」换成「有没有她够得着的东西」**:`if not seen and not verbs`。
两样都没有的照旧跳过 —— 放开成"只要在场就进"会让纯摆设白占提示词预算,
而「没声明 = 感知不到」那条默认值会从背面被拆掉(反向用例钉着这一半)。

⚠️ **这是一次行为变更,契约没加格**:已有世界的提示词可能多出几行,
而多出来的正是"从前她看得见却够不着"的那些东西。回执 `docs/FOR-STUDIO.md` §3.57。
...
创作台 `../tool/docs/引擎接口诉求-插件.md` 欠的第一条问的是:一条越界写、一条没声明的
...
(platform `../platform/docs/工作台-运维台契约.md` §4)。v2 时代运维台是从解包出来的
