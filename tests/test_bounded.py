...
def test_the_prompt_does_not_grow_as_the_world_ages(tmp_path, redis):
    ...
    try:
        ...
        # 记忆确实在写。**判据不能是"涨满 50"** —— 容量上限是"能存几条",不是
        # "必须存满几条":夜间固化(橱窗点亮了 `memory.consolidation.enabled`)会在
        # 日切时衰减+清扫,记忆准入闸(默认关,橱窗有意没点,见 REFERENCE §2.9.13)
        # 开着时还会挡掉复读 —— 两样都让这个数**该**低于容量。拿"涨满"当后端在写
        # 的证据,等于要求那些重复行必须存在,而它们正是这两层要消灭的东西。
