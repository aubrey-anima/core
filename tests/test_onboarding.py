...
# ── the runtime is an API, not a site ────────────────────────────────────────
...
def _app(tmp_path, **kwargs):
    ...
    return scheduler, create_app(scheduler, run_loop=False, **kwargs)


def test_root_explains_itself_instead_of_404ing_blankly(tmp_path):
    """The engine has no UI and never will — the player surface belongs to the
    site, authoring to the studio. Whoever opens this in a browser deserves a
    sentence, not an empty 404."""
    ...
    try:
        ...
        assert "anima-studio" in r.json()["detail"]
        assert "/api/v1/world" in r.json()["api"]
    finally:
        scheduler.stop()


@pytest.mark.parametrize("path", ["/api/chat", "/api/v1/chat", "/api/player/move"])
def test_name_based_player_routes_are_gone(tmp_path, path):
    """They only ever served the deleted demo page; the site talks /internal/v1,
    where identity comes from a signed membership claim rather than a typed name."""
    from fastapi.testclient import TestClient

    scheduler, app = _app(tmp_path)
    try:
        with TestClient(app) as client:
            assert client.post(path, json={"agent_id": "夏", "text": "hi"}).status_code == 404
    finally:
        scheduler.stop()
...
def test_find_free_port_walks_past_a_taken_one():
    import socket
    ...
    with socket.socket() as taken:
        ...
        taken.listen(1)
        assert onboarding.find_free_port("127.0.0.1", port) == port + 1


def test_browsable_url_never_hands_out_0_0_0_0():
    assert onboarding.browsable_url("0.0.0.0", 8000) == "http://127.0.0.1:8000"
