...
def select_cast(candidates: Iterable[dict[str, Any]], *, cast_pool: Sequence[str] = (),
    ...
    · `taken` 是**别人那条开着的线上的人**(3.13.0)——同样是软的,而
      **两道软闸分级退让**:先两道都上,挤不下就**先丢 `recent`、还保着
      `taken`**,都筛空了才整份还回去。次序的理由是这两道闸不一样重 ——
      `taken` 守的是产品命题(两个玩家走出两条不一样的线),`recent` 守的是
      同一个玩家眼里的新鲜感;**挤不下的时候先丢新鲜感**。
      ⚠️ 最后那一步一个字没改:**空 cast 的下场是沉默**,比撞脸坏得多。
    ...
    #
    # 🔴 **两道软闸是分级退让的,不是一起上一起放**(3.13.0,验收 A 三轮 ⑤,
    # 调度台裁)。上一版把 `recent` 和 `taken` 并成一个集合一次筛,筛空了整份
    # 还回去 —— 而**小池子上那一步几乎总是走到**:三个角色的世界,一个被硬闸
    # 筛掉,剩两个正好一个是"上一拍派过的"、一个是"别人线上占着的",
    # 于是两个玩家又撞回同一个人。
    # 次序的理由是**这两道闸不一样重**:`taken` 守的是产品命题
    # (两个玩家走出两条不一样的线),`recent` 守的是同一个玩家眼里的新鲜感。
    # 挤不下的时候,**先丢新鲜感**。
    # ⚠️ 最后那一步一个字没改:**都筛空了就整份还回去** ——
    # 「宁可撞脸,不可沉默」是这一层的硬纪律,空 cast 会让编剧这一拍什么都不写。
    for names in (
        # ① 两道都上
        (*recent, *taken),
        # ② 挤不下就先丢 `recent`,还保着 `taken`
        tuple(taken),
    ):
        stale = {str(a) for a in names if str(a) and str(a) != str(keep)}
        if not stale:
            continue
