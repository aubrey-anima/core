...
class Projection:
    ...
    **事件日志**,这一格只是它的投影。刷新一次页面就重生成一段的话,同一个时刻的
    ...
    #: 🆕 3.13.0(批 3c §2.1):**这个地方最近有人做过事**是第几条。
    #: `{地点 id: seq}` —— 交织那一格读它:A 当着 B 的面做一件事,
    #: B 那一屏就该开口。
    #: ⚠️ **按地点记,不按人记**:「撞见」的语义是**同一个地方**,
    #: 而"谁撞见了谁"是读的时候才知道的(B 此刻在哪)。
    #: 🔴 **不是一份跨玩家的故事状态** —— 它只回答「这儿刚有人动过手没有」,
    #: 一个数;两条线仍然各是各的(任务单 §2.1:交织的价值在「被撞见」,
    #: 不在「共享一份剧情」)。
    place_move_seq: dict[str, int] = field(default_factory=dict)
