...
# 关着时行为与从前逐位相同。当初不敢开的理由(3.2.0,2026-08)是引擎侧收紧会当场
# 打断线上世界:`player_move` 是宿主可选调用,而那时**线上一个宿主都没调**,
# 于是"异地"会是每一次调用的默认值。
# ⚠️ **那句话现在过期了**:站点已于 2026-08-13 前后接上 `player_move`(落脚、
    ...
    **地点是参数,不是引擎替玩家挑的。** `leave` 从前把目的地取成 `point_ids()[0]`
    ...
    def _colocation_refusal(
        ...
        **默认不生效**(`presence.enforce_colocation`)。这不是犹豫,是账:引擎侧
        收紧会**当场打断线上世界** —— `player_move` 是宿主可选调用,而 **3.2.0
        那会儿(2026-08 上旬)线上一个宿主都没调**,于是"异地"是每一次调用的
        默认值,一开就是 `give` 全线开始拒绝。迁移的次序只能是"先让宿主调
        `player_move`,再开这个开关"。

        ⚠️ **上面那句实况已经过期,留着是为了记账,不是为了引用**:站点已于
        2026-08-13 前后接上(落脚 / 重连 / 世界重启复位三处),晚潮那个世界
        30 个活跃玩家里 26 个有 `home_location`。所以今天的真实形状不是"没人
        调",而是**在场记录 15 分钟不续就过期**(`api._PLAYER_TTL_SECONDS`)——
        这道闸开起来会挡住的是"最近一刻钟没进过世界的人"。一个验收员照着上面
        那句旧话推出过"这扇门在线上一次都不会开"的结论,而那是反的:门开着,
        只是对最近进过世界的人开。**要开这个开关,先量一次那个覆盖率。**
    ...
    def _together(
        ...
        **「带我去某个地方」也是一起做的一件事**,而且是玩家最先想得到的那一件 ——
        ...
        try:
            ...
            # 刚说的事 —— 而那封信接不接得出来是宿主的事(`World.invitations()`
            # 是 3.6.0 才有的门,不是每个宿主都接了),接不出来的世界里这句话
            # 就什么也没发生。**她点他的名那条路一个字不变**(`tools/body.py` 的
