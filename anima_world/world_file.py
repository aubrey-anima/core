...
# 那几个**不是一列条目而是一个对象**的段,和 `config` 同类:一份而不是一条。
AUTHOR_OBJECT_TYPES: dict[str, str] = {
    "config": "config",
    "mock_narration": "mock_narration",
    # 🆕 3.11.0(玩法层批 3a):**给实时编剧的「指导」。作者层第十六个段。**
    #
    # 老板 2026-09-02:「这个周更只能算是指导」。周更包从"我替玩家写好第 3 天
    # 发生什么"变成"我告诉编剧:这个世界讲的是什么、谁可以被叫出来、哪几件事
    # 绝对不许发生、多快、以及三个该往哪儿使劲的路口" —— **主线由玩家自己走**。
    #
    # 对象型(一份而不是一条),和 `pack` / `config` 同类:一个世界只有一份
    # 当下的指导,"一个文件里两份指导"在这一层根本表达不出来。
    #
    # ⚠️ **和 `beat` / `plugin` / `edge` / `pack` 逐字同一种情形**:老引擎见到
    # 不认识的 `type` 是**开不了机的硬失败**,所以带 `guidance` 记录的包
    # `engine_min` 必须写 `3.11.0`。
    "guidance": "guidance",
    # 🆕 3.10.0(周更链路 2a-①):**内容包的身份。作者层第十五个段。**
    #
    # 「这份内容是第几周的」要有一个身份 —— 装 / 升 / 停用 / 审计 /「本周更新」
    # 全都按它,而**事后补不回来**:一份已经装进去的包没有 id,谁也说不清那三拍
    # 是哪一周加的(「推迟功能可以,推迟数据不行」)。
    #
    # 🔴 **为什么是这里、不是 manifest 的一格**:`WorldFileManifest.from_record` 把
    # 不认识的键收进 `extra`,于是一支老引擎见到 manifest 上的 `pack` 会**静默忽略
    # 身份而照装内容** —— 包进去了、`disable` 再也找不到它。而作者层的 `type` 是
    # 闭集:老引擎见到不认识的 `type` 是**开不了机的硬失败**,和 `beat`(3.7.0)/
    # `plugin` / `edge`(3.8.0)逐字同一种,**响的**。所以带 `pack` 记录的包
    # `engine_min` 必须写 `3.10.0`。
    #
    # 🔴 **为什么是"对象型"而不是"一列条目"**:**一个文件就是一个 pack**,
    # 这份包里的作者记录全归属它。写成列表就表达得出"一个文件里两个包",
    # 而那时"这条记录属于哪一个"要么靠每条记录再加一格(十四个段各改一次),
    # 要么靠顺序(靠顺序 = 靠作者自觉)。**分成这一种形状让它不可表达。**
    "pack": "pack",
}
