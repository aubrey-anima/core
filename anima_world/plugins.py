...
def install_plugins(
    ...
    **升级 = 同 id 更高 version**:声明里没了的事实**裁剪**(删值、删可见性行)并
    ...
    # 新数据"在那儿仍然是一次真的降级。
    if on_boot:
        # ⚠️ **这一支从 3.11.3 起在正常开机路上基本打不着**(3.12.0,验收 ⑤):
        # 真正做决定的是 `__main__._plugin_bodies` —— 它在**装之前**就把过期的
        # 那几份筛掉了(那一处才决定"运行时装的是哪一份",而这一层只管
        # "别拿旧声明覆盖库里那一行")。
        # 留着它**不是冗余**:`install_plugins` 是公开函数,直接调它的调用方
        # (以及以后别的入口)拿不到那一层的保护。
        # 🔴 但**日志那一句改说在做决定的那一处** —— 一句在这儿印不出来的话,
        # 会让人以为"没跳过任何东西"。
        kept: list[str] = []
        fresh: list[Any] = []
        for plugin in plugins:
            ...
            fresh.append(plugin)
        if kept:
            # 🔴 warning 不是 info:运维台按 warning 抓日志。
            # ⚠️ 走到这儿说明调用方**绕过了** `_plugin_bodies`(见上),
            # 所以措辞要和那一处分得开 —— 两句一模一样的话会让人查错地方。
            logger.warning(
                "这几个插件库里那份更新,这一趟不覆盖库里那一行:%s —— "
